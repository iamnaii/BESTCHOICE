#!/usr/bin/env bash
# v4.9 ความจุที่ลูกค้าอยากได้ไม่มี → บอก+เทียบเรท — รัน apply-capacity-not-available-v4-9.sql
# — รัน apply-capacity-not-available-v4-9.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-capacity-not-available-v4-9.sql || die "apply ล้มเหลว"
