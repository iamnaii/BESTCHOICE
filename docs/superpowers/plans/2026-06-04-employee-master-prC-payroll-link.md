...
- **D4 — PII: mask `employeeTaxId` (= nationalId) for non-OWNER/ACCOUNTANT on BOTH the create response AND the read path (F1, from scrutinize; owner: "เก็บกวาดให้สุด" 2026-06-05).** The snapshot taxId === the employee's `nationalId`; OWNER/ACCOUNTANT see it full, BRANCH_MANAGER/FINANCE_MANAGER get it masked via the shared `maskPayrollTaxIds` helper — applied in `createPayroll`'s response (Step 3b) and in `findOne` / `GET /expense-documents/:id` (Step 3c–3e). Otherwise a draft payroll OR a payroll-detail GET becomes a way to enumerate national IDs around PR-A's PII gate. `list` exposes no payroll lines (safe). The STORED value stays full (for ภงด.1).
