#!/usr/bin/env bash
# v3.5 สอนบอทใช้บันทึกสถานะ + ลูกค้าเก่ากลับมา + [รูป ...] — รัน apply-state-note-v3-5.sql
# — รัน apply-state-note-v3-5.sql (เขียนทับทั้งก้อนแบบ md5-guard)
# รัน:  bash scripts/ops/apply-state-note-v3-5.sh
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-state-note-v3-5.sql || die "apply ล้มเหลว"
