-- บอทขาย: ปรับจากแชทจริง 19-22 ก.ย. + โหมดไม่มีสต๊อก + รูปตารางผ่อน (คำสั่งเจ้าของ 2026-09-22) — ถอย
...
BEGIN
  ...
  IF md5(ex) <> '1b958573b505bbebf62595b81049a282' THEN RAISE EXCEPTION 'EXTRAS บน DB ไม่ใช่ฉบับที่คาด (ฉบับ 2026-09-22 — มีคนแก้ persona หลัง apply) md5=%', md5(ex); END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:age-requirement$K$ AND deleted_at IS NULL AND (priority <> 100 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$อายุ$K$,$K$กี่ปี$K$,$K$ปีเท่าไหร่$K$,$K$อายุเท่าไหร่$K$,$K$17$K$,$K$18$K$,$K$19$K$,$K$ไม่ถึง 18$K$,$K$เด็ก$K$,$K$นักเรียน$K$,$K$ผู้ปกครอง$K$,$K$ค้ำ$K$,$K$ค้ำประกัน$K$]::text[] OR response_template <> $K$อายุ 18 ปีขึ้นไป ทำสัญญาเองได้เลยค่ะ
...
นักศึกษา มีผู้ปกครองค้ำให้ค่า$K$)) THEN RAISE EXCEPTION 'แถว KB faq:age-requirement ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:device-lock$K$ AND deleted_at IS NULL AND (priority <> 100)) THEN RAISE EXCEPTION 'แถว KB faq:device-lock ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:early-payoff$K$ AND deleted_at IS NULL AND (priority <> 100)) THEN RAISE EXCEPTION 'แถว KB faq:early-payoff ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:late-fee$K$ AND deleted_at IS NULL AND (priority <> 100)) THEN RAISE EXCEPTION 'แถว KB faq:late-fee ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:no-payslip-freelance$K$ AND deleted_at IS NULL AND (priority <> 100)) THEN RAISE EXCEPTION 'แถว KB faq:no-payslip-freelance ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:paid-still-locked$K$ AND deleted_at IS NULL AND (priority <> 100)) THEN RAISE EXCEPTION 'แถว KB faq:paid-still-locked ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:payment-channel-reminder$K$ AND deleted_at IS NULL AND (priority <> 100)) THEN RAISE EXCEPTION 'แถว KB faq:payment-channel-reminder ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$promo:imported-free-down$K$ AND deleted_at IS NULL AND (priority <> 100 OR response_template <> $K$โปรฟรีดาวน์ iPhone มือสอง เครื่องนอก (ของแท้ Apple โมเดลต่างประเทศ ประกันร้าน 30 วัน)
...
ขอคืนได้ก่อนชำระงวดแรก แต่ต้องจ่ายงวดแรก 1 งวด เครื่องต้องสภาพเดิม ครบกล่องอุปกรณ์ ออก iCloud แล้ว$K$)) THEN RAISE EXCEPTION 'แถว KB promo:imported-free-down ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:imported-device$K$ AND deleted_at IS NULL AND (priority <> 100)) THEN RAISE EXCEPTION 'แถว KB faq:imported-device ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:imported-tradein$K$ AND deleted_at IS NULL AND (priority <> 100)) THEN RAISE EXCEPTION 'แถว KB faq:imported-tradein ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:price_installment$K$ AND deleted_at IS NULL AND (priority <> 40 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$ราคา$K$,$K$ราคาเต็ม$K$,$K$กี่บาท$K$]::text[])) THEN RAISE EXCEPTION 'แถว KB extracted:price_installment ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:product_availability$K$ AND deleted_at IS NULL AND (priority <> 40 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$เหลือ$K$,$K$สีไหน$K$,$K$มีของ$K$,$K$ของหมด$K$,$K$มีสีอะไร$K$]::text[])) THEN RAISE EXCEPTION 'แถว KB extracted:product_availability ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:installment_terms$K$ AND deleted_at IS NULL AND (priority <> 40 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$เรท$K$,$K$แบบไหน$K$,$K$กี่งวด$K$,$K$กี่เดือน$K$]::text[])) THEN RAISE EXCEPTION 'แถว KB extracted:installment_terms ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:approval_process$K$ AND deleted_at IS NULL AND (priority <> 60 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$อนุมัติ$K$,$K$อายุงาน$K$,$K$บูโร$K$,$K$เช็คบูโร$K$,$K$ติดบูโร$K$,$K$แบล็คลิสต์$K$,$K$เครดิตไม่ดี$K$]::text[])) THEN RAISE EXCEPTION 'แถว KB extracted:approval_process ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:store_location_hours$K$ AND deleted_at IS NULL AND (trigger_keywords IS DISTINCT FROM ARRAY[$K$ที่ไหน$K$,$K$ที่อยู่$K$,$K$อยุ่$K$,$K$แถว$K$,$K$เปิด$K$,$K$ปิด$K$,$K$กี่โมง$K$,$K$ร้านอยู่$K$,$K$บขส$K$,$K$แผนที่$K$,$K$พิกัด$K$,$K$ทางไป$K$,$K$ไปยังไง$K$]::text[])) THEN RAISE EXCEPTION 'แถว KB extracted:store_location_hours ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
END $G$;
...
- ของแท้/iCloud → ตอบชัดว่าไม่ติด iCloud + มีหน้าร้านจริงที่ลพบุรี$P$,
  $P$- ของแท้/iCloud → ตอบชัดว่าไม่ติด iCloud + มีหน้าร้านจริงที่ลพบุรี$P$), updated_at = NOW()
WHERE key = 'shop_bot_persona_bot_extras' AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 0, trigger_keywords = ARRAY[$K$อายุ$K$,$K$กี่ปี$K$,$K$ปีเท่าไหร่$K$,$K$อายุเท่าไหร่$K$,$K$18$K$,$K$19$K$,$K$ยังไม่ 20$K$,$K$ไม่ถึง 20$K$,$K$เด็ก$K$,$K$นักเรียน$K$,$K$ผู้ปกครอง$K$,$K$ค้ำ$K$,$K$ค้ำประกัน$K$]::text[], response_template = $K$อายุ 20 ปีขึ้นไป ทำสัญญาเองได้เลยค่ะ
...
UPDATE chat_knowledge_base SET priority = 0, updated_at = NOW() WHERE id = $K$faq:device-lock$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 0, updated_at = NOW() WHERE id = $K$faq:early-payoff$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 0, updated_at = NOW() WHERE id = $K$faq:late-fee$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 0, updated_at = NOW() WHERE id = $K$faq:no-payslip-freelance$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 0, updated_at = NOW() WHERE id = $K$faq:paid-still-locked$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 0, updated_at = NOW() WHERE id = $K$faq:payment-channel-reminder$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 85, response_template = $K$โปรฟรีดาวน์ iPhone มือสอง เครื่องนอก (ของแท้ Apple โมเดลต่างประเทศ ประกันร้าน 30 วัน)
iPhone 13 128GB ผ่อนเดือนละ 1,758 บาท 12 งวด
iPhone 14 128GB ผ่อนเดือนละ 1,885 บาท 12 งวด
iPhone 15 128GB ผ่อนเดือนละ 2,395 บาท 12 งวด
iPhone 16 128GB ผ่อนเดือนละ 2,631 บาท 12 งวด
...
UPDATE chat_knowledge_base SET priority = 60, updated_at = NOW() WHERE id = $K$faq:imported-device$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 55, updated_at = NOW() WHERE id = $K$faq:imported-tradein$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 100, trigger_keywords = ARRAY[$K$ราคา$K$,$K$ดาว$K$,$K$ผ่อน$K$,$K$เงินเดือน$K$,$K$เท่าไร$K$,$K$เท่าไหร่$K$]::text[], updated_at = NOW() WHERE id = $K$extracted:price_installment$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 100, trigger_keywords = ARRAY[$K$มี$K$,$K$มั้ย$K$,$K$มีไหม$K$,$K$เหลือ$K$,$K$ยัง$K$,$K$สีไหน$K$]::text[], updated_at = NOW() WHERE id = $K$extracted:product_availability$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 89, trigger_keywords = ARRAY[$K$ผ่อน$K$,$K$เรท$K$,$K$แบบไหน$K$,$K$เดือน$K$,$K$งวด$K$]::text[], updated_at = NOW() WHERE id = $K$extracted:installment_terms$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 78, trigger_keywords = ARRAY[$K$อนุมัติ$K$,$K$เช็ค$K$,$K$ได้ไหม$K$,$K$ผ่าน$K$,$K$ได้รับ$K$,$K$อายุงาน$K$]::text[], updated_at = NOW() WHERE id = $K$extracted:approval_process$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET trigger_keywords = ARRAY[$K$มือ$K$,$K$มือ1$K$,$K$มือ2$K$,$K$แบต$K$,$K$สภาพ$K$,$K$ผ่าน$K$]::text[], updated_at = NOW() WHERE id = $K$extracted:second_hand_condition$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET trigger_keywords = ARRAY[$K$ที่ไหน$K$,$K$ที่อยู่$K$,$K$อยุ่$K$,$K$แถว$K$,$K$เปิด$K$,$K$ปิด$K$,$K$กี่โมง$K$]::text[], updated_at = NOW() WHERE id = $K$extracted:store_location_hours$K$ AND deleted_at IS NULL;
UPDATE system_config SET deleted_at = NOW(), updated_at = NOW() WHERE key IN ('shop_bot_rate_cards','shop_bot_page_autoreply_markers','shop_bot_stock_mode') AND deleted_at IS NULL;

-- ผลตรวจ (ต้อง ✓ ทุกบรรทัด)
SELECT CASE WHEN md5(value) = '4c0fb09ec182f519f9f7c1dd3426d043' THEN '✓' ELSE '✗' END || ' EXTRAS ตรงฉบับที่คาดทุกตัวอักษร len=' || length(value) FROM system_config WHERE key='shop_bot_persona_bot_extras' AND deleted_at IS NULL;
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:age-requirement' FROM chat_knowledge_base WHERE id = $K$faq:age-requirement$K$ AND deleted_at IS NULL AND NOT (priority <> 0 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$อายุ$K$,$K$กี่ปี$K$,$K$ปีเท่าไหร่$K$,$K$อายุเท่าไหร่$K$,$K$18$K$,$K$19$K$,$K$ยังไม่ 20$K$,$K$ไม่ถึง 20$K$,$K$เด็ก$K$,$K$นักเรียน$K$,$K$ผู้ปกครอง$K$,$K$ค้ำ$K$,$K$ค้ำประกัน$K$]::text[] OR response_template <> $K$อายุ 20 ปีขึ้นไป ทำสัญญาเองได้เลยค่ะ
17-19 ผ่อนได้ แต่มีผู้ปกครองมาเซ็นด้วยวันรับเครื่อง
ต่ำกว่า 17 ยังทำสัญญาไม่ได้ค่ะ
นักศึกษา มีผู้ปกครองค้ำให้ค่า$K$);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:device-lock' FROM chat_knowledge_base WHERE id = $K$faq:device-lock$K$ AND deleted_at IS NULL AND NOT (priority <> 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:early-payoff' FROM chat_knowledge_base WHERE id = $K$faq:early-payoff$K$ AND deleted_at IS NULL AND NOT (priority <> 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:late-fee' FROM chat_knowledge_base WHERE id = $K$faq:late-fee$K$ AND deleted_at IS NULL AND NOT (priority <> 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:no-payslip-freelance' FROM chat_knowledge_base WHERE id = $K$faq:no-payslip-freelance$K$ AND deleted_at IS NULL AND NOT (priority <> 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:paid-still-locked' FROM chat_knowledge_base WHERE id = $K$faq:paid-still-locked$K$ AND deleted_at IS NULL AND NOT (priority <> 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:payment-channel-reminder' FROM chat_knowledge_base WHERE id = $K$faq:payment-channel-reminder$K$ AND deleted_at IS NULL AND NOT (priority <> 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB promo:imported-free-down' FROM chat_knowledge_base WHERE id = $K$promo:imported-free-down$K$ AND deleted_at IS NULL AND NOT (priority <> 85 OR response_template <> $K$โปรฟรีดาวน์ iPhone มือสอง เครื่องนอก (ของแท้ Apple โมเดลต่างประเทศ ประกันร้าน 30 วัน)
iPhone 13 128GB ผ่อนเดือนละ 1,758 บาท 12 งวด
iPhone 14 128GB ผ่อนเดือนละ 1,885 บาท 12 งวด
iPhone 15 128GB ผ่อนเดือนละ 2,395 บาท 12 งวด
iPhone 16 128GB ผ่อนเดือนละ 2,631 บาท 12 งวด
...
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:imported-device' FROM chat_knowledge_base WHERE id = $K$faq:imported-device$K$ AND deleted_at IS NULL AND NOT (priority <> 60);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:imported-tradein' FROM chat_knowledge_base WHERE id = $K$faq:imported-tradein$K$ AND deleted_at IS NULL AND NOT (priority <> 55);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:price_installment' FROM chat_knowledge_base WHERE id = $K$extracted:price_installment$K$ AND deleted_at IS NULL AND NOT (priority <> 100 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$ราคา$K$,$K$ดาว$K$,$K$ผ่อน$K$,$K$เงินเดือน$K$,$K$เท่าไร$K$,$K$เท่าไหร่$K$]::text[]);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:product_availability' FROM chat_knowledge_base WHERE id = $K$extracted:product_availability$K$ AND deleted_at IS NULL AND NOT (priority <> 100 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$มี$K$,$K$มั้ย$K$,$K$มีไหม$K$,$K$เหลือ$K$,$K$ยัง$K$,$K$สีไหน$K$]::text[]);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:installment_terms' FROM chat_knowledge_base WHERE id = $K$extracted:installment_terms$K$ AND deleted_at IS NULL AND NOT (priority <> 89 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$ผ่อน$K$,$K$เรท$K$,$K$แบบไหน$K$,$K$เดือน$K$,$K$งวด$K$]::text[]);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:approval_process' FROM chat_knowledge_base WHERE id = $K$extracted:approval_process$K$ AND deleted_at IS NULL AND NOT (priority <> 78 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$อนุมัติ$K$,$K$เช็ค$K$,$K$ได้ไหม$K$,$K$ผ่าน$K$,$K$ได้รับ$K$,$K$อายุงาน$K$]::text[]);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:second_hand_condition' FROM chat_knowledge_base WHERE id = $K$extracted:second_hand_condition$K$ AND deleted_at IS NULL AND NOT (trigger_keywords IS DISTINCT FROM ARRAY[$K$มือ$K$,$K$มือ1$K$,$K$มือ2$K$,$K$แบต$K$,$K$สภาพ$K$,$K$ผ่าน$K$]::text[]);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:store_location_hours' FROM chat_knowledge_base WHERE id = $K$extracted:store_location_hours$K$ AND deleted_at IS NULL AND NOT (trigger_keywords IS DISTINCT FROM ARRAY[$K$ที่ไหน$K$,$K$ที่อยู่$K$,$K$อยุ่$K$,$K$แถว$K$,$K$เปิด$K$,$K$ปิด$K$,$K$กี่โมง$K$]::text[]);
SELECT CASE WHEN count(*) = 0 THEN '✓' ELSE '✗' END || ' ปิด system_config 3 ค่าแล้ว' FROM system_config WHERE key IN ('shop_bot_rate_cards','shop_bot_page_autoreply_markers','shop_bot_stock_mode') AND deleted_at IS NULL;
COMMIT;
