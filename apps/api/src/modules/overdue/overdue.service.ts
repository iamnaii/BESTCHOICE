...
export class OverdueService {
  ...
  async logContact(
    ...
    if (dto.result === 'PROMISED') {
      ...
      // Settlement-date guards (mirror recordSettlement). ป้องกัน:
      //  - นัดย้อนหลัง = ปิดบัง aging bucket
      //  - นัดไกล > 30 วัน = staff ยืดเวลาลูกหนี้, และ promise tab filter
      //    (now+30d ใน queue.service) ทำให้ promise หาย
      //  - นัดงวด 2 ก่อนงวด 1 = schedule ย้อนเวลา
      if (dto.settlementDate) {
        ...
        if (dto.secondSettlementDate) {
          const second = new Date(dto.secondSettlementDate);
          if (isNaN(second.getTime())) {
            throw new BadRequestException('วันนัดชำระงวด 2 ไม่ถูกต้อง');
          }
          if (second.getTime() <= promised.getTime()) {
            throw new BadRequestException('วันนัดชำระงวด 2 ต้องอยู่หลังงวดแรก');
          }
          if (second.getTime() > maxDate.getTime()) {
            throw new BadRequestException(
              `วันนัดชำระงวด 2 ห่างจากวันนี้เกิน ${PROMISED_MAX_DAYS} วัน`,
            );
          }
        }
      }
    ...
    const [callLog] = await this.prisma.$transaction([
      this.prisma.callLog.create({
        data: {
          ...
          settlementAmount:
            dto.settlementAmount != null ? new Prisma.Decimal(dto.settlementAmount) : null,
          secondSettlementDate: dto.secondSettlementDate
            ? new Date(dto.secondSettlementDate)
            : null,
          secondSettlementAmount:
            dto.secondSettlementAmount != null
              ? new Prisma.Decimal(dto.secondSettlementAmount)
              : null,
   ...
   *   2. คำนวณ outstanding ใหม่หลังรับเงิน (= outstanding-before − amountPaid, จาก credit balance)
   *   3. สร้าง CallLog ใหม่ result=PROMISED ด้วย settlementAmount=outstanding-after,
   *      settlementDate=newSettlementDate (= ส่วนที่ลูกค้านัดจ่ายต่อ)
   *
  ...
  async partialPaymentReschedule(
    ...
    // otherwise paid==outstanding never trips and the PROMISED log carries
    // the entire remaining loan as settlementAmount.
    const now = new Date();
    const contract = await this.prisma.contract.findFirst({
      where: { id: contractId, deletedAt: null },
      include: {
    ...
    // — applies to both full-pay and partial-pay paths, including the case
    // where logContact threw and the catch swallowed it.
    this.kpiService.invalidate();
   ...
   * (BrokenPromiseCron เป็นตัวสร้าง entry — runs hourly)
   */
   ...
   *   - LEGAL → set dunningStage='LEGAL_ACTION' + AuditLog
  ...
  async escalate(
    ...
    // SALES/BRANCH_MANAGER can still trigger LETTER/MDM here.
