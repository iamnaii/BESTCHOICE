...
| เกิน per-room reply cap (`ai.autoMaxRepliesPerSession`, นับตลอดอายุห้อง ไม่ reset) | Haiku ตอบต่อไป | เงียบ + พนักงานเห็น unread (ไม่มี handoff flag) | semantics เดียวกับ Facebook — ทีมทดสอบจะเจอใน session แรกๆ อย่าเข้าใจผิดว่าระบบพัง; ปรับ cap ได้จากหน้า Settings |
