#!/usr/bin/env bash
# ปลด AI ที่โดนปิดจากข้อความทักทายอัตโนมัติ — รัน apply-no-payment-invite-v5-1.sql
# — รัน apply-no-payment-invite-v5-1.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-no-payment-invite-v5-1.sql || die "apply ล้มเหลว"
