#!/usr/bin/env bash
# แก้ EXTRAS v2 (2026-08-14): ปิดช่อง "เสนอรุ่นสำรองก่อนเวลา" + ถอดข้ออ้างปักธงอัตโนมัติ 3 จุด
# — รัน apply-substitute-after-quote-only.sql (เขียนทับทั้งก้อนแบบ md5-guard)
# รัน:  bash scripts/ops/apply-substitute-after-quote-only.sh
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-substitute-after-quote-only.sql || die "apply ล้มเหลว"
