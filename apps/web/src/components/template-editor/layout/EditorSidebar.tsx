    ...
    <div className="w-[200px] min-h-full bg-primary-950 flex flex-col shrink-0">
      ...
      <div className="px-4 py-4 border-b border-white/10">
        {onBack && (
          <button
            onClick={onBack}
            className="flex items-center gap-1.5 text-white/70 hover:text-white text-sm mb-2 transition-colors"
          >
            <ArrowLeft size={14} />
            กลับหน้าหลัก
          </button>
        ...
        <p className="text-white/50 text-xs">Document Template Editor</p>
      </div>
      ...
      <div className="px-4 py-3 border-t border-white/10">
        <p className="text-white/50 text-xs">BESTCHOICEPHONE Co., Ltd.</p>
      </div>
    </div>
  );
}
