...
describe('DOC-11 sticker 50×30 mm regression — /stickers print geometry through headless Chromium', () => {
  ...
  beforeAll(async () => {
    ...
    const specs = [
      { label: 'USED-A', category: 'PHONE_USED', model: `iPhone 14 ${TEST_NAME_PREFIX} สติกเกอร์ A`, batteryHealth: 91, warrantyDays: 180 },
      { label: 'USED-B', category: 'PHONE_USED', model: `Galaxy S23 ${TEST_NAME_PREFIX} สติกเกอร์ B`, batteryHealth: 84, warrantyDays: null },
      { label: 'NEW-C', category: 'PHONE_NEW', model: `iPhone 15 ${TEST_NAME_PREFIX} สติกเกอร์ C`, batteryHealth: null, warrantyDays: 365 },
    ] as const;
    for (const spec of specs) {
      const row = await h.prisma.product.create({ data: {
        name: `${TEST_NAME_PREFIX} ${spec.model}`, brand: spec.model.startsWith('Galaxy') ? 'Samsung' : 'Apple', model: spec.model, storage: '128GB', color: 'ดำ', category: spec.category,
  ...
  describe('1440px', () => {
    ...
    it('3 products → /stickers?productIds → print media → one 50×30 mm page per sticker, TH Sarabun PSK only, model on each page, text inside the sticker', async () => {
      ...
      for (const [index, product] of products.entries()) {
        const text = foldThai(pageText(pdf.pages[index]));
        expect({ page: index + 1, hasModel: text.includes(foldThai(product.model)) }).toEqual({ page: index + 1, hasModel: true });
      }
      ...
      const artifacts = [shotPreview, saveArtifact(DOMAIN, 'stickers-3-1440.pdf', bytes).relativePath, saveArtifact(DOMAIN, 'stickers-3-1440.json', JSON.stringify({ pageCount: pdf.pageCount, fonts: pdf.fonts, sizes, pages: pdf.pages.map((p) => ({ w: p.widthPt.toFixed(2), h: p.heightPt.toFixed(2), lines: p.lines })), dataRequests }, null, 2)).relativePath];
      expect(errors.filter((e) => e.startsWith('pageerror'))).toEqual([]);
      recordScenario(DOMAIN, scenario({ id: `${DOMAIN}/print-geometry`, title: `/stickers?productIds=(3) → print media → ${pdf.pageCount} pages of ${STICKER_WIDTH_PT} × ${STICKER_HEIGHT_PT} pt (50 × 30 mm), fonts ${pdf.fonts.join('/')}, text sizes ${sizes.map((s) => `${s.size}pt×${s.count}`).join(' ')}, every sticker carries its model, no text outside the page`, routes: ['GET /api/sticker-templates/products/data', 'GET /api/products/stock'], artifacts, unverified: ['thermal printer output / label stock alignment (DOC-12)', 'the OS print dialog (headless page.pdf() stands in)'], notes: `${consoleNote(errors)}; this branch predates the sticker redesign merged to main on 2026-09-11 (PR #1575) — nothing about prices or wording is asserted so the check holds on both versions` }));
    }, 300000);
  });
});
