#!/usr/bin/env bash
# v4.8 อธิบายความต่างเรทไปแล้ว ห้ามพูดซ้ำ — รัน apply-no-repeat-rate-diff-v4-8.sql
# — รัน apply-no-repeat-rate-diff-v4-8.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-no-repeat-rate-diff-v4-8.sql || die "apply ล้มเหลว"
