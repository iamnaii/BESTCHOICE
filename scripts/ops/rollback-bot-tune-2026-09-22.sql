-- บอทขาย: ปรับจากแชทจริง 19-22 ก.ย. (รอบ 2: สังเคราะห์ 458 ห้อง) + โหมดไม่มีสต๊อก + รูปตารางผ่อน (คำสั่งเจ้าของ 2026-09-22) — ถอย
...
BEGIN
  ...
  IF md5(ex) <> '4afbc8660a743caa8277e90712f1e2db' THEN RAISE EXCEPTION 'EXTRAS บน DB ไม่ใช่ฉบับที่คาด (ฉบับ 2026-09-22 — มีคนแก้ persona หลัง apply) md5=%', md5(ex); END IF;
  ...
  IF (length(bs) - length(replace(bs, $P$- ขายเครื่องใหม่ + มือสองคัดสภาพ (มือสองเครื่องไทยประกันร้าน 60 วัน · มือสองเครื่องนอกประกันร้าน 30 วัน) · รับซื้อ/รับเทิร์นเฉพาะ iPhone 12 ถึงรุ่นล่าสุด ไม่รับ mini (ถามขาย/เทิร์นรุ่นเก่ากว่า 12 → "ร้านรับซื้อกับรับเทิร์นเฉพาะ iPhone 12 ขึ้นไปค่ะ" ห้ามบอกให้เอาเครื่องมาให้ทีมดู ห้ามเดาราคา · ไม่ได้ถาม = ไม่ต้องพูดถึง)$P$, ''))) / length($P$- ขายเครื่องใหม่ + มือสองคัดสภาพ (มือสองเครื่องไทยประกันร้าน 60 วัน · มือสองเครื่องนอกประกันร้าน 30 วัน) · รับซื้อ/รับเทิร์นเฉพาะ iPhone 12 ถึงรุ่นล่าสุด ไม่รับ mini (ถามขาย/เทิร์นรุ่นเก่ากว่า 12 → "ร้านรับซื้อกับรับเทิร์นเฉพาะ iPhone 12 ขึ้นไปค่ะ" ห้ามบอกให้เอาเครื่องมาให้ทีมดู ห้ามเดาราคา · ไม่ได้ถาม = ไม่ต้องพูดถึง)$P$) <> 1 THEN RAISE EXCEPTION 'BASE บน DB ไม่มีบรรทัดที่ชั้นนี้แก้ (จุด 1) แบบฉบับ 2026-09-22 — มีคนแก้บรรทัดนี้หลัง apply พอดี 1 ครั้ง'; END IF;
  IF position($P$- ขายเครื่องใหม่ + มือสองคัดสภาพ (มือสองเครื่องไทยประกันร้าน 60 วัน · มือสองเครื่องนอกประกันร้าน 30 วัน) · รับเทิร์น iPhone 12 ถึงรุ่นล่าสุด (รุ่นเก่ากว่านั้น tool จะไม่คืนราคาเทิร์น — ตอบว่า "รุ่นนี้ต้องให้ทีมดูเครื่องที่ร้านก่อนค่ะ" ห้ามเดาราคา)$P$ in bs) > 0 THEN RAISE EXCEPTION 'BASE บน DB มีบรรทัดร่างโปรฟรีดาวน์ (ถอยแล้ว?) อยู่แล้ว (จุด 1)'; END IF;
...
นักศึกษา มีผู้ปกครองค้ำให้ค่า$K$)) THEN RAISE EXCEPTION 'แถว KB faq:age-requirement ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:device-lock$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 100)) THEN RAISE EXCEPTION 'แถว KB faq:device-lock ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:early-payoff$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 100)) THEN RAISE EXCEPTION 'แถว KB faq:early-payoff ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:late-fee$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 100)) THEN RAISE EXCEPTION 'แถว KB faq:late-fee ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:paid-still-locked$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 100)) THEN RAISE EXCEPTION 'แถว KB faq:paid-still-locked ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:payment-channel-reminder$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 100)) THEN RAISE EXCEPTION 'แถว KB faq:payment-channel-reminder ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
...
ขอคืนได้ก่อนชำระงวดแรก แต่ต้องจ่ายงวดแรก 1 งวด เครื่องต้องสภาพเดิม ครบกล่องอุปกรณ์ ออก iCloud แล้ว$K$)) THEN RAISE EXCEPTION 'แถว KB promo:imported-free-down ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:imported-device$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 100 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$เครื่องนอก$K$,$K$eSIM$K$,$K$อีซิม$K$,$K$ใส่ซิม$K$,$K$ซิมไทย$K$,$K$ซิมอะไร$K$,$K$ซิมคู่$K$,$K$2 ซิม$K$,$K$ค่ายไหน$K$,$K$ชัตเตอร์$K$,$K$โมเดลต่างประเทศ$K$,$K$เครื่องนอกโมเดล$K$,$K$รหัสรุ่น$K$,$K$LL$K$,$K$ZP$K$,$K$J/A$K$,$K$KH/A$K$,$K$CH/A$K$,$K$โมเดลจีน$K$,$K$เครื่องหิ้ว$K$]::text[] OR example_questions IS DISTINCT FROM ARRAY[$K$เครื่องนอกคืออะไร$K$,$K$เครื่องนอกใส่ซิมไทยได้ไหม$K$,$K$เครื่องนอกใช้ซิมอะไรได้$K$,$K$เครื่องนอกโมเดลประเทศอะไร$K$]::text[] OR response_template IS DISTINCT FROM $K$เครื่องนอกคือ iPhone แท้ของ Apple โมเดลต่างประเทศค่ะ ไม่ติด iCloud
...
ทีมงานเปิดรหัสรุ่นให้ดู และลองโทรออกกับเปิดเน็ตกับเครื่องจริงก่อนส่งมอบค่ะ$K$)) THEN RAISE EXCEPTION 'แถว KB faq:imported-device ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$faq:imported-tradein$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 100)) THEN RAISE EXCEPTION 'แถว KB faq:imported-tradein ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:price_installment$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 40 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$ราคา$K$,$K$ราคาเต็ม$K$,$K$กี่บาท$K$]::text[])) THEN RAISE EXCEPTION 'แถว KB extracted:price_installment ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:product_availability$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 40 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$เหลือ$K$,$K$สีไหน$K$,$K$มีของ$K$,$K$ของหมด$K$,$K$มีสีอะไร$K$,$K$มีไหม$K$]::text[])) THEN RAISE EXCEPTION 'แถว KB extracted:product_availability ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:installment_terms$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 40 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$เรท$K$,$K$แบบไหน$K$,$K$กี่งวด$K$,$K$กี่เดือน$K$]::text[])) THEN RAISE EXCEPTION 'แถว KB extracted:installment_terms ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:approval_process$K$ AND deleted_at IS NULL AND (priority IS DISTINCT FROM 60 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$อนุมัติ$K$,$K$อนุมัติไว$K$,$K$รู้ผล$K$,$K$รอผล$K$,$K$รอนาน$K$,$K$กี่นาที$K$,$K$อายุงาน$K$]::text[] OR example_questions IS DISTINCT FROM ARRAY[$K$อนุมัติเร็วไหม$K$,$K$รู้ผลกี่นาที$K$]::text[] OR response_template IS DISTINCT FROM $K$ไม่ต้องมีบัตรเครดิต ไม่เช็คบูโรค่ะ 😊
ส่งเอกสารตามเรทที่เลือกในแชทนี้ได้เลย
ทีมงานเช็คให้ในเวลาทำการ 10:00-19:00
รู้ผลไวใน 5 นาทีค่ะ$K$)) THEN RAISE EXCEPTION 'แถว KB extracted:approval_process ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM chat_knowledge_base WHERE id = $K$extracted:store_location_hours$K$ AND deleted_at IS NULL AND (trigger_keywords IS DISTINCT FROM ARRAY[$K$ที่ไหน$K$,$K$ที่อยู่$K$,$K$อยุ่$K$,$K$แถว$K$,$K$เปิด$K$,$K$กี่โมง$K$,$K$ร้านอยู่$K$,$K$บขส$K$,$K$แผนที่$K$,$K$พิกัด$K$,$K$ทางไป$K$,$K$ไปยังไง$K$,$K$ร้านปิด$K$,$K$ปิดกี่$K$,$K$ปิดยัง$K$,$K$ปิดหรือยัง$K$,$K$ที่ตั้ง$K$,$K$โลเคชั่น$K$,$K$โลเคชัน$K$,$K$วันหยุด$K$,$K$โมงเย็น$K$,$K$ทุ่ม$K$,$K$เข้าไปดู$K$,$K$เข้าไปได้$K$,$K$ไปถึง$K$,$K$เข้าร้าน$K$,$K$เบอร์ร้าน$K$,$K$หาร้าน$K$,$K$หาไม่เจอ$K$]::text[] OR example_questions IS DISTINCT FROM ARRAY[$K$ร้านอยู่ที่ไหน$K$,$K$เปิดกี่โมง$K$,$K$เป็นที่ไหนของลพบุรี$K$,$K$ขอโลเคชั่นร้านหน่อย$K$,$K$วันนี้เข้าไปได้ไหม$K$,$K$ขอเบอร์ร้านหน่อย$K$]::text[])) THEN RAISE EXCEPTION 'แถว KB extracted:store_location_hours ไม่ตรงค่าที่คาด — มีคนแก้หลังตรวจ'; END IF;
...
- ลูกค้าพิมพ์เลขบัตร/ที่อยู่/รหัสไฟล์/ข้อมูลคนอื่น → ห้ามทวน ห้ามใส่ productNote$P$,
  ...
  แล้วเดินขั้น 7 ต่อ (ห้ามตัดสินผลเอกสาร) · บริบทอื่น → ขอบคุณแล้วถามต่อว่ารูปนี้คือรุ่นที่สนใจ/สลิปอะไรคะ$P$), updated_at = NOW()
WHERE key = 'shop_bot_persona_bot_extras' AND deleted_at IS NULL;
UPDATE system_config SET value = replace(value,
  $P$- ขายเครื่องใหม่ + มือสองคัดสภาพ (มือสองเครื่องไทยประกันร้าน 60 วัน · มือสองเครื่องนอกประกันร้าน 30 วัน) · รับซื้อ/รับเทิร์นเฉพาะ iPhone 12 ถึงรุ่นล่าสุด ไม่รับ mini (ถามขาย/เทิร์นรุ่นเก่ากว่า 12 → "ร้านรับซื้อกับรับเทิร์นเฉพาะ iPhone 12 ขึ้นไปค่ะ" ห้ามบอกให้เอาเครื่องมาให้ทีมดู ห้ามเดาราคา · ไม่ได้ถาม = ไม่ต้องพูดถึง)$P$,
  $P$- ขายเครื่องใหม่ + มือสองคัดสภาพ (มือสองเครื่องไทยประกันร้าน 60 วัน · มือสองเครื่องนอกประกันร้าน 30 วัน) · รับเทิร์น iPhone 12 ถึงรุ่นล่าสุด (รุ่นเก่ากว่านั้น tool จะไม่คืนราคาเทิร์น — ตอบว่า "รุ่นนี้ต้องให้ทีมดูเครื่องที่ร้านก่อนค่ะ" ห้ามเดาราคา)$P$), updated_at = NOW()
WHERE key = 'shop_bot_persona_base' AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 0, trigger_keywords = ARRAY[$K$อายุ$K$,$K$กี่ปี$K$,$K$ปีเท่าไหร่$K$,$K$อายุเท่าไหร่$K$,$K$18$K$,$K$19$K$,$K$ยังไม่ 20$K$,$K$ไม่ถึง 20$K$,$K$เด็ก$K$,$K$นักเรียน$K$,$K$ผู้ปกครอง$K$,$K$ค้ำ$K$,$K$ค้ำประกัน$K$]::text[], example_questions = ARRAY[]::text[], response_template = $K$อายุ 20 ปีขึ้นไป ทำสัญญาเองได้เลยค่ะ
...
UPDATE chat_knowledge_base SET priority = 0, trigger_keywords = ARRAY[$K$สลิป$K$,$K$ไม่มีสลิป$K$,$K$สลิปเงินเดือน$K$,$K$ฟรีแลนซ์$K$,$K$แม่ค้า$K$,$K$พ่อค้า$K$,$K$ขายของออนไลน์$K$,$K$รับจ้าง$K$,$K$อาชีพอิสระ$K$,$K$ไม่มีเงินเดือน$K$,$K$ไม่มีรายได้ประจำ$K$,$K$ทำงานอิสระ$K$]::text[], example_questions = ARRAY[]::text[], response_template = $K$ไม่ต้องมีสลิป ไม่ต้องมีบัตรเครดิตค่ะ 😊
...
UPDATE chat_knowledge_base SET priority = 85, trigger_keywords = ARRAY[$K$เครื่องนอก$K$,$K$ฟรีดาวน์$K$,$K$ฟรี ดาวน์$K$,$K$ไม่ต้องดาวน์$K$,$K$ไม่มีดาวน์$K$,$K$ไม่วางดาวน์$K$,$K$โปรเครื่องนอก$K$]::text[], example_questions = ARRAY[$K$เครื่องนอกผ่อนเดือนละเท่าไหร่$K$,$K$มีโปรฟรีดาวน์ไหม$K$,$K$ฟรีดาวน์มีรุ่นไหนบ้าง$K$]::text[], response_template = $K$โปรฟรีดาวน์ iPhone มือสอง เครื่องนอก (ของแท้ Apple โมเดลต่างประเทศ ประกันร้าน 30 วัน)
iPhone 13 128GB ผ่อนเดือนละ 1,758 บาท 12 งวด
iPhone 14 128GB ผ่อนเดือนละ 1,885 บาท 12 งวด
iPhone 15 128GB ผ่อนเดือนละ 2,395 บาท 12 งวด
iPhone 16 128GB ผ่อนเดือนละ 2,631 บาท 12 งวด
...
UPDATE chat_knowledge_base SET priority = 60, trigger_keywords = ARRAY[$K$เครื่องนอก$K$,$K$eSIM$K$,$K$อีซิม$K$,$K$ชัตเตอร์$K$,$K$โมเดลต่างประเทศ$K$,$K$เครื่องหิ้ว$K$,$K$ของแท้$K$,$K$ของปลอม$K$]::text[], example_questions = ARRAY[$K$เครื่องนอกคืออะไร$K$,$K$เครื่องนอกของแท้ไหม$K$,$K$เครื่องนอกใส่ซิมไทยได้ไหม$K$]::text[], response_template = $K$เครื่องนอกคือ iPhone แท้ของ Apple ที่ผลิตขายในต่างประเทศค่ะ ไม่ติด iCloud
เช็ครหัสรุ่นในเครื่องได้ที่ ตั้งค่า > ทั่วไป > เกี่ยวกับ
...
ทีมงานเปิดเครื่องจริงให้เช็คและลองซิมที่ร้านก่อนตัดสินใจค่ะ$K$, updated_at = NOW() WHERE id = $K$faq:imported-device$K$ AND deleted_at IS NULL;
...
UPDATE chat_knowledge_base SET priority = 78, trigger_keywords = ARRAY[$K$อนุมัติ$K$,$K$เช็ค$K$,$K$ได้ไหม$K$,$K$ผ่าน$K$,$K$ได้รับ$K$,$K$อายุงาน$K$]::text[], example_questions = ARRAY[$K$เช็คเครดิตไหม$K$,$K$อนุมัติเร็วไหม$K$,$K$จะต้องเช็คบูโรไหม$K$]::text[], response_template = $K$ไม่ต้องมีบัตรเครดิต ไม่เช็คบูโรค่า ใช้บัตรประชาชนก็ผ่อนได้เลยค่ะพี่ 😊
อนุมัติไวใน 5 นาทีค่ะ

สะดวกส่งเอกสารให้เช็คเลยมั้ยคะ$K$, updated_at = NOW() WHERE id = $K$extracted:approval_process$K$ AND deleted_at IS NULL;
...
UPDATE chat_knowledge_base SET trigger_keywords = ARRAY[$K$ที่ไหน$K$,$K$ที่อยู่$K$,$K$อยุ่$K$,$K$แถว$K$,$K$เปิด$K$,$K$ปิด$K$,$K$กี่โมง$K$]::text[], example_questions = ARRAY[$K$ร้านอยู่ที่ไหน$K$,$K$เปิดกี่โมง$K$,$K$เป็นที่ไหนของลพบุรี$K$]::text[], updated_at = NOW() WHERE id = $K$extracted:store_location_hours$K$ AND deleted_at IS NULL;
UPDATE chat_knowledge_base SET priority = 90, trigger_keywords = ARRAY[$K$จัดส่ง$K$,$K$ส่งของ$K$,$K$ส่งได้ไหม$K$,$K$ส่งต่างจังหวัด$K$,$K$เก็บปลายทาง$K$,$K$ปลายทาง$K$,$K$ems$K$,$K$ไปรษณีย์$K$,$K$kerry$K$,$K$flash$K$,$K$มารับ$K$,$K$รับเครื่อง$K$,$K$รับของ$K$]::text[], example_questions = ARRAY[$K$ส่งของได้ไหม$K$,$K$เก็บเงินปลายทางได้ไหม$K$,$K$ส่งต่างจังหวัดไหม$K$,$K$ต้องไปรับที่ร้านไหม$K$]::text[], response_template = $K$ตอนนี้ร้านยังไม่มีบริการจัดส่งนะคะ ลูกค้าเข้ามารับเครื่องที่หน้าร้านได้เลยค่า
...
UPDATE chat_knowledge_base SET deleted_at = NOW(), updated_at = NOW() WHERE id = $K$faq:credit-history$K$ AND intent = $K$credit_history$K$ AND deleted_at IS NULL;
...
SELECT CASE WHEN position($P$- ขายเครื่องใหม่ + มือสองคัดสภาพ (มือสองเครื่องไทยประกันร้าน 60 วัน · มือสองเครื่องนอกประกันร้าน 30 วัน) · รับเทิร์น iPhone 12 ถึงรุ่นล่าสุด (รุ่นเก่ากว่านั้น tool จะไม่คืนราคาเทิร์น — ตอบว่า "รุ่นนี้ต้องให้ทีมดูเครื่องที่ร้านก่อนค่ะ" ห้ามเดาราคา)$P$ in value) > 0 AND position($P$- ขายเครื่องใหม่ + มือสองคัดสภาพ (มือสองเครื่องไทยประกันร้าน 60 วัน · มือสองเครื่องนอกประกันร้าน 30 วัน) · รับซื้อ/รับเทิร์นเฉพาะ iPhone 12 ถึงรุ่นล่าสุด ไม่รับ mini (ถามขาย/เทิร์นรุ่นเก่ากว่า 12 → "ร้านรับซื้อกับรับเทิร์นเฉพาะ iPhone 12 ขึ้นไปค่ะ" ห้ามบอกให้เอาเครื่องมาให้ทีมดู ห้ามเดาราคา · ไม่ได้ถาม = ไม่ต้องพูดถึง)$P$ in value) = 0 THEN '✓' ELSE '✗' END || ' BASE จุด 1 เป็นฉบับที่คาด' FROM system_config WHERE key='shop_bot_persona_base' AND deleted_at IS NULL;
SELECT CASE WHEN md5(value) = 'db81460176ea792872d2fa7855094d7d' THEN '✓ BASE ตรงฉบับที่คาดทุกตัวอักษร' ELSE 'ℹ BASE มีจุดอื่นที่ถูกแก้แยกจากชั้นนี้ (คงไว้)' END || ' len=' || length(value) FROM system_config WHERE key='shop_bot_persona_base' AND deleted_at IS NULL;
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:age-requirement' FROM chat_knowledge_base WHERE id = $K$faq:age-requirement$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 0 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$อายุ$K$,$K$กี่ปี$K$,$K$ปีเท่าไหร่$K$,$K$อายุเท่าไหร่$K$,$K$18$K$,$K$19$K$,$K$ยังไม่ 20$K$,$K$ไม่ถึง 20$K$,$K$เด็ก$K$,$K$นักเรียน$K$,$K$ผู้ปกครอง$K$,$K$ค้ำ$K$,$K$ค้ำประกัน$K$]::text[] OR example_questions IS DISTINCT FROM ARRAY[]::text[] OR response_template IS DISTINCT FROM $K$อายุ 20 ปีขึ้นไป ทำสัญญาเองได้เลยค่ะ
17-19 ผ่อนได้ แต่มีผู้ปกครองมาเซ็นด้วยวันรับเครื่อง
ต่ำกว่า 17 ยังทำสัญญาไม่ได้ค่ะ
นักศึกษา มีผู้ปกครองค้ำให้ค่า$K$);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:device-lock' FROM chat_knowledge_base WHERE id = $K$faq:device-lock$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:early-payoff' FROM chat_knowledge_base WHERE id = $K$faq:early-payoff$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:late-fee' FROM chat_knowledge_base WHERE id = $K$faq:late-fee$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:paid-still-locked' FROM chat_knowledge_base WHERE id = $K$faq:paid-still-locked$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:payment-channel-reminder' FROM chat_knowledge_base WHERE id = $K$faq:payment-channel-reminder$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 0);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:no-payslip-freelance' FROM chat_knowledge_base WHERE id = $K$faq:no-payslip-freelance$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 0 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$สลิป$K$,$K$ไม่มีสลิป$K$,$K$สลิปเงินเดือน$K$,$K$ฟรีแลนซ์$K$,$K$แม่ค้า$K$,$K$พ่อค้า$K$,$K$ขายของออนไลน์$K$,$K$รับจ้าง$K$,$K$อาชีพอิสระ$K$,$K$ไม่มีเงินเดือน$K$,$K$ไม่มีรายได้ประจำ$K$,$K$ทำงานอิสระ$K$]::text[] OR example_questions IS DISTINCT FROM ARRAY[]::text[] OR response_template IS DISTINCT FROM $K$ไม่ต้องมีสลิป ไม่ต้องมีบัตรเครดิตค่ะ 😊
ฟรีแลนซ์ แม่ค้าออนไลน์ รับจ้าง ผ่อนได้หมด
มีเงินเข้าบัญชี → สเตทเม้นท์ 3 เดือน (เรทที่ 1)
ไม่มี → รูปตอนทำงาน (เรทที่ 2)$K$);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB promo:imported-free-down' FROM chat_knowledge_base WHERE id = $K$promo:imported-free-down$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 85 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$เครื่องนอก$K$,$K$ฟรีดาวน์$K$,$K$ฟรี ดาวน์$K$,$K$ไม่ต้องดาวน์$K$,$K$ไม่มีดาวน์$K$,$K$ไม่วางดาวน์$K$,$K$โปรเครื่องนอก$K$]::text[] OR example_questions IS DISTINCT FROM ARRAY[$K$เครื่องนอกผ่อนเดือนละเท่าไหร่$K$,$K$มีโปรฟรีดาวน์ไหม$K$,$K$ฟรีดาวน์มีรุ่นไหนบ้าง$K$]::text[] OR response_template IS DISTINCT FROM $K$โปรฟรีดาวน์ iPhone มือสอง เครื่องนอก (ของแท้ Apple โมเดลต่างประเทศ ประกันร้าน 30 วัน)
iPhone 13 128GB ผ่อนเดือนละ 1,758 บาท 12 งวด
iPhone 14 128GB ผ่อนเดือนละ 1,885 บาท 12 งวด
iPhone 15 128GB ผ่อนเดือนละ 2,395 บาท 12 งวด
iPhone 16 128GB ผ่อนเดือนละ 2,631 บาท 12 งวด
...
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:imported-device' FROM chat_knowledge_base WHERE id = $K$faq:imported-device$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 60 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$เครื่องนอก$K$,$K$eSIM$K$,$K$อีซิม$K$,$K$ชัตเตอร์$K$,$K$โมเดลต่างประเทศ$K$,$K$เครื่องหิ้ว$K$,$K$ของแท้$K$,$K$ของปลอม$K$]::text[] OR example_questions IS DISTINCT FROM ARRAY[$K$เครื่องนอกคืออะไร$K$,$K$เครื่องนอกของแท้ไหม$K$,$K$เครื่องนอกใส่ซิมไทยได้ไหม$K$]::text[] OR response_template IS DISTINCT FROM $K$เครื่องนอกคือ iPhone แท้ของ Apple ที่ผลิตขายในต่างประเทศค่ะ ไม่ติด iCloud
เช็ครหัสรุ่นในเครื่องได้ที่ ตั้งค่า > ทั่วไป > เกี่ยวกับ
บางโมเดลมีข้อจำกัด: โมเดลอเมริการุ่น 14 ขึ้นไปใช้ eSIM อย่างเดียว · โมเดลญี่ปุ่น/เกาหลีปิดเสียงชัตเตอร์ไม่ได้ · โมเดลฮ่องกงใส่ 2 ซิมแต่ไม่มี eSIM
ทีมงานเปิดเครื่องจริงให้เช็คและลองซิมที่ร้านก่อนตัดสินใจค่ะ$K$);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB faq:imported-tradein' FROM chat_knowledge_base WHERE id = $K$faq:imported-tradein$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 55);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:price_installment' FROM chat_knowledge_base WHERE id = $K$extracted:price_installment$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 100 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$ราคา$K$,$K$ดาว$K$,$K$ผ่อน$K$,$K$เงินเดือน$K$,$K$เท่าไร$K$,$K$เท่าไหร่$K$]::text[]);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:product_availability' FROM chat_knowledge_base WHERE id = $K$extracted:product_availability$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 100 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$มี$K$,$K$มั้ย$K$,$K$มีไหม$K$,$K$เหลือ$K$,$K$ยัง$K$,$K$สีไหน$K$]::text[]);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:installment_terms' FROM chat_knowledge_base WHERE id = $K$extracted:installment_terms$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 89 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$ผ่อน$K$,$K$เรท$K$,$K$แบบไหน$K$,$K$เดือน$K$,$K$งวด$K$]::text[]);
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:approval_process' FROM chat_knowledge_base WHERE id = $K$extracted:approval_process$K$ AND deleted_at IS NULL AND NOT (priority IS DISTINCT FROM 78 OR trigger_keywords IS DISTINCT FROM ARRAY[$K$อนุมัติ$K$,$K$เช็ค$K$,$K$ได้ไหม$K$,$K$ผ่าน$K$,$K$ได้รับ$K$,$K$อายุงาน$K$]::text[] OR example_questions IS DISTINCT FROM ARRAY[$K$เช็คเครดิตไหม$K$,$K$อนุมัติเร็วไหม$K$,$K$จะต้องเช็คบูโรไหม$K$]::text[] OR response_template IS DISTINCT FROM $K$ไม่ต้องมีบัตรเครดิต ไม่เช็คบูโรค่า ใช้บัตรประชาชนก็ผ่อนได้เลยค่ะพี่ 😊
อนุมัติไวใน 5 นาทีค่ะ

สะดวกส่งเอกสารให้เช็คเลยมั้ยคะ$K$);
...
SELECT CASE WHEN count(*) = 1 THEN '✓' ELSE '✗' END || ' KB extracted:store_location_hours' FROM chat_knowledge_base WHERE id = $K$extracted:store_location_hours$K$ AND deleted_at IS NULL AND NOT (trigger_keywords IS DISTINCT FROM ARRAY[$K$ที่ไหน$K$,$K$ที่อยู่$K$,$K$อยุ่$K$,$K$แถว$K$,$K$เปิด$K$,$K$ปิด$K$,$K$กี่โมง$K$]::text[] OR example_questions IS DISTINCT FROM ARRAY[$K$ร้านอยู่ที่ไหน$K$,$K$เปิดกี่โมง$K$,$K$เป็นที่ไหนของลพบุรี$K$]::text[]);
...
SELECT CASE WHEN count(*) = 0 THEN '✓' ELSE '✗' END || ' ปิดแถว KB ใหม่ faq:credit-history แล้ว' FROM chat_knowledge_base WHERE id = $K$faq:credit-history$K$ AND deleted_at IS NULL;
