#!/usr/bin/env bash
# v2.3 แก้ประโยค "มี" ชน "สั่งเข้า" — รัน apply-no-mee-vs-order-conflict.sql
# — รัน apply-no-mee-vs-order-conflict.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-no-mee-vs-order-conflict.sql || die "apply ล้มเหลว"
