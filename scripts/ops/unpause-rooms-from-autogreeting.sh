#!/usr/bin/env bash
# ปลด AI ที่โดนปิดจากข้อความทักทายอัตโนมัติ — รัน unpause-rooms-from-autogreeting.sql
# — รัน unpause-rooms-from-autogreeting.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/unpause-rooms-from-autogreeting.sql || die "apply ล้มเหลว"
