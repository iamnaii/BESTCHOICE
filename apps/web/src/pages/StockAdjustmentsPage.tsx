...
const reasonLabels: Record<string, { label: string; className: string }> = {
  ...
  CORRECTION: { label: 'แก้ไขข้อมูล', className: 'bg-blue-100 text-blue-700' },
  WRITE_OFF: { label: 'ตัดจำหน่าย', className: 'bg-gray-100 text-gray-700' },
  OTHER: { label: 'อื่นๆ', className: 'bg-gray-100 text-gray-500' },
};
...
export default function StockAdjustmentsPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (a: StockAdjustment) => (
        <div>
          <div className="font-medium text-gray-900 text-sm">{a.product.brand} {a.product.model}</div>
          {a.product.imeiSerial && (
            <div className="text-xs text-gray-400 font-mono">{a.product.imeiSerial}</div>
          )}
        </div>
      ),
    },
    {
      ...
      label: 'สาเหตุ',
      render: (a: StockAdjustment) => {
        const r = reasonLabels[a.reason] || { label: a.reason, className: 'bg-gray-100 text-gray-700' };
    ...
    },
    {
      key: 'previousStatus',
      label: 'สถานะเดิม',
      render: (a: StockAdjustment) => <span className="text-xs text-gray-500">{a.previousStatus}</span>,
    },
    {
      key: 'notes',
      label: 'หมายเหตุ',
      render: (a: StockAdjustment) => (
        <span className="text-xs text-gray-500 max-w-[200px] truncate block">{a.notes || '-'}</span>
      ),
    },
    {
      key: 'costPrice',
    ...
    },
    {
      key: 'adjustedBy',
      label: 'ผู้ปรับ',
      render: (a: StockAdjustment) => <span className="text-xs text-gray-500">{a.adjustedBy.name}</span>,
    },
  ];
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-lg border p-4 border-l-4 border-l-red-500">
              <div className="text-xs text-gray-500 mb-1">มูลค่ารวมที่ปรับ</div>
              <div className="text-2xl font-bold text-red-600">{summary.totalValue.toLocaleString()} ฿</div>
            </div>
          </div>

          <div className="bg-white rounded-lg border p-5">
            <h2 className="text-sm font-semibold text-gray-700 mb-4">สรุปตามสาเหตุ</h2>
            <div className="space-y-3">
              {Object.entries(summary.byReason).map(([reason, data]) => {
                const r = reasonLabels[reason] || { label: reason, className: 'bg-gray-100 text-gray-700' };
                ...
                return (
                  <div key={reason} className="flex items-center gap-4">
                    ...
                    <span className="text-sm text-gray-500 w-28 text-right">{data.totalValue.toLocaleString()} ฿</span>
                  </div>
                );
              })}
              {Object.keys(summary.byReason).length === 0 && (
                <div className="text-sm text-gray-400 text-center py-4">ยังไม่มีข้อมูล</div>
              )}
            </div>
          </div>
        </div>
              ...
              <div className="mt-1 border rounded-lg max-h-40 overflow-y-auto">
                {searchProducts.products.slice(0, 10).map((p) => (
                  ...
                  >
                    ...
                    {p.imeiSerial && <span className="text-xs text-gray-400 ml-2 font-mono">{p.imeiSerial}</span>}
