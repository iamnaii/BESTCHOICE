...
function DataGridColumnHeader<TData, TValue>({
  ...
  const headerLabel = () => {
    return (
      <div
        className={cn(
          'text-accent-foreground font-normal inline-flex h-full items-center gap-1.5 text-[0.8125rem] [&_svg]:size-3.5 [&_svg]:opacity-60',
          className,
        )}
      >
        {icon && icon}
            ...
            {column.getCanSort() && (
              <>
                ...
                >
                  ...
                  <span className="grow">น้อยไปมาก</span>
                  {column.getIsSorted() === 'asc' && <Check className="size-4 text-primary" />}
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => {
                ...
                >
                  ...
                  <span className="grow">มากไปน้อย</span>
                  {column.getIsSorted() === 'desc' && <Check className="size-4 text-primary" />}
                </DropdownMenuItem>
              </>
            )}
            ...
            {props.tableLayout?.columnsPinnable && column.getCanPin() && (
              <>
                <DropdownMenuItem onClick={() => column.pin(column.getIsPinned() === 'left' ? false : 'left')}>
                  ...
                  <span className="grow">ตรึงซ้าย</span>
                  {column.getIsPinned() === 'left' && <Check className="size-4 text-primary" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => column.pin(column.getIsPinned() === 'right' ? false : 'right')}>
                  ...
                  <span className="grow">ตรึงขวา</span>
                  {column.getIsPinned() === 'right' && <Check className="size-4 text-primary" />}
                </DropdownMenuItem>
              </>
            )}
