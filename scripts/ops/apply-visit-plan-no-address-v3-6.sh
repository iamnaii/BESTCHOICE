#!/usr/bin/env bash
# v3.6 ถามแผนเข้าร้านก่อนขอชื่อเบอร์ + ห้ามถามที่อยู่จัดส่ง — รัน apply-visit-plan-no-address-v3-6.sql
# — รัน apply-visit-plan-no-address-v3-6.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-visit-plan-no-address-v3-6.sql || die "apply ล้มเหลว"
