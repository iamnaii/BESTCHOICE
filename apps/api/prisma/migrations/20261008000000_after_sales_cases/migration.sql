...
-- หมายเหตุ (fix round 1, R6): repair_tickets.created_at เป็น timestamp(3) without time zone ที่เก็บเวลา UTC ตรงตัว
-- ต้อง reinterpret เป็น UTC ก่อน แล้วค่อยแปลงเป็น Asia/Bangkok — AT TIME ZONE ชั้นเดียวจะเลื่อนผิดทาง
INSERT INTO "after_sales_cases" ("id","case_number","branch_id","customer_id","source","contract_id","product_id","device_brand","device_model","device_imei","device_serial","symptom","warranty_snapshot","outcome","repair_ticket_id","replacement_contract_id","stage","received_by_id","received_at","created_at","updated_at","closed_at","cancelled_at")
SELECT gen_random_uuid()::text,
  'AS-' || to_char((rt.created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Bangkok', 'YYYYMMDD') || '-' || lpad((row_number() OVER (PARTITION BY ((rt.created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Bangkok')::date ORDER BY rt.created_at))::text, 4, '0'),
  ...
  CASE WHEN rt.status = 'REPLACED' THEN 'SAME_MODEL_EXCHANGE'::"AfterSalesOutcome" ELSE 'REPAIR'::"AfterSalesOutcome" END,
  rt.id, rt.replacement_contract_id,
