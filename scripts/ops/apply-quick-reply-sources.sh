#!/usr/bin/env bash
# รัน apply-quick-reply-sources.sql (ปุ่ม quick replies v2.1)
# — รัน apply-quick-reply-sources.sql (เขียนทับทั้งก้อนแบบ md5-guard)
# รัน:  bash scripts/ops/apply-quick-reply-sources.sh
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-quick-reply-sources.sql || die "apply ล้มเหลว"
