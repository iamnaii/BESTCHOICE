#!/usr/bin/env bash
# v4.0-1 เปลี่ยนชื่อบอทเป็น "แอดมิน" (คำสั่งเจ้าของ) — รัน apply-admin-name-v4-0.sql
# — รัน apply-admin-name-v4-0.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-admin-name-v4-0.sql || die "apply ล้มเหลว"
