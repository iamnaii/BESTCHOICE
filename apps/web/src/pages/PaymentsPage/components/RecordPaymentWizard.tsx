...
import { ContractInfoPanel, CaseBadge, type DetectedCase } from './PaymentContractSummary';
...
const METHOD_OPTIONS: { id: WizardMethod; label: string; icon: React.ReactNode; desc: string }[] = [
  { id: 'CASH', label: 'เงินสด', icon: <Banknote className="size-4" />, desc: 'รับเงินสดโดยตรง' },
  {
    ...
    desc: 'ลูกค้าโอนเอง · กรอก ref + slip',
  },
  {
    id: 'QR',
    label: 'ชำระผ่าน QR',
    icon: <QrCode className="size-4" />,
    desc: 'ส่ง QR ให้ลูกค้าใน LINE',
  },
  {
    id: 'CARD',
    label: 'บัตร',
    icon: <CreditCard className="size-4" />,
    desc: 'เครื่อง EDC · เงินเข้าบัญชีธนาคาร',
  },
];
...
function detectCase(
  ...
  rescheduleAdvanceBalance: Decimal = new Decimal(0),
  isLastInstallment: boolean = false,
): DetectedCase {
  ...
  // Park bucket (พักงวดสุดท้าย) joins the generic advance ONLY on the last
  // installment — owner directive 2026-08-16.
  const effPark = isLastInstallment ? rescheduleAdvanceBalance : new Decimal(0);
  const effAdvance = consumeAdvance ? advanceBalance.plus(effPark) : new Decimal(0);
...
export function RecordPaymentWizard({
  ...
  // พักงวดสุดท้าย — reschedule-fee (6a/6b) prepayment, separate bucket, consumable
  // ONLY when this installment IS the contract's last (owner directive 2026-08-16).
  const rescheduleAdvanceBalance = useMemo(
    () => new Decimal(payment.contract.rescheduleAdvanceBalance ?? 0),
    [payment.contract.rescheduleAdvanceBalance],
  );
  const isLastInstallment = payment.installmentNo === payment.contract.totalMonths;
  ...
  const isPreviewReady: boolean =
    ...
    // Allow 0 cash when advance (+ park bucket on the last installment) fully
    // covers the installment (detectCase = NORMAL)
    (debouncedParams.amountReceived > 0 ||
      (detectedCase === 'NORMAL' &&
        advanceBalance
          .plus(isLastInstallment ? rescheduleAdvanceBalance : new Decimal(0))
          .gte(expectedTotal))) &&
  ...
  const sendQrMutation = useMutation({
    ...
    onError: (err: unknown) => {
      const msg = err instanceof Error ? err.message : 'ส่ง QR ไม่สำเร็จ — ลองอีกครั้ง';
      toast.error(msg);
    },
  });
        ...
        >
          ...
          <DialogHeader className="px-6 pt-4 pb-2.5 shrink-0 border-b border-border">
            <DialogTitle className="text-base font-semibold leading-snug">
              บันทึกชำระ — {payment.contract.contractNumber} / {payment.contract.customer.name} —
            ...
            <div className="grid grid-cols-[1fr_340px] gap-4 items-start">
              ...
              <div className="space-y-3 min-w-0">
                ...
                {/* Advance balance banner — shown when contract has advance to consume,
                  or (on the last installment only) a park-bucket credit. */}
                {(advanceBalance.gt(0) ||
                  (isLastInstallment && rescheduleAdvanceBalance.gt(0))) && (
                  <AdvanceBalanceBanner
                    ...
                    rescheduleAdvanceBalance={rescheduleAdvanceBalance}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      // so "ปกติ" doesn't stay highlighted when the typed amount is partial/advance.
                      {
                        ...
                        label: 'ปกติ',
                        toggle: true,
                        onClick: () => setCaseOverride(null),
                        active: apiCase !== 'PARTIAL' && apiCase !== 'OVERPAY_ADVANCE',
                      },
                      {
                        key: 'PARTIAL',
                        label: 'แบ่งชำระ',
                        toggle: true,
                        onClick: () => setCaseOverride('PARTIAL'),
                        active: apiCase === 'PARTIAL',
                      },
                      {
                        key: 'OVERPAY_ADVANCE',
                        label: 'ล่วงหน้า',
                        toggle: true,
                        onClick: () => setCaseOverride('OVERPAY_ADVANCE'),
                        active: apiCase === 'OVERPAY_ADVANCE',
                      },
                      {
                        key: 'PAYOFF',
                        label: 'ปิดยอด',
                        toggle: false,
                        onClick: () => setShowPayoffOverlay(true),
                        active: false,
                      },
                      {
                        key: 'RESCHEDULE',
                    ...
                    {[
                      {
                        ...
                        apply: () => netDue.toFixed(2),
                      },
                      {
                        key: 'roundup',
                        label: 'ปิดขึ้น',
                        value: netDueRoundedUp.toFixed(0),
                        apply: () => netDueRoundedUp.toFixed(2),
                      },
                      { key: 'custom', label: 'กำหนดเอง', value: 'ระบุจำนวน', apply: null },
                    ].map((tile) => {
                      const active =
                        tile.apply != null &&
                        new Decimal(amountReceived || 0).eq(new Decimal(tile.apply()));
                      return (
                        <button
                          key={tile.key}
                          type="button"
                  ...
                  <div className="rounded-lg border border-info/30 bg-info/5 px-3 py-3 flex items-start gap-2.5">
                    ...
                    <div className="text-xs text-foreground leading-relaxed">
                      ...
                      <span className="text-muted-foreground">
                        เลขอ้างอิง + หลักฐานบันทึกอัตโนมัติจาก PaySolutions webhook · QR หมดอายุ 24
                        ชม. · ลูกค้าจ่ายเสร็จระบบบันทึก payment ให้ทันที
                      </span>
                    </div>
                  </div>
                )}
          ...
          <DialogFooter className="px-6 py-3 border-t border-border shrink-0 flex items-center justify-between">
            <Button
              variant="outline"
              onClick={onClose}
              disabled={isSubmitting || sendQrMutation.isPending}
            >
              ...
              >
                {sendQrMutation.isPending ? (
                  ...
                  <>
                    ...
                    ส่ง QR ให้ลูกค้าสแกนจ่าย ฿
                    {receivedNum.toLocaleString('th-TH', {
