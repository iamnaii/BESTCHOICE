...
| D1.1.3.5 | effective_date support | P0 | ✅ | this PR | Stacked on D1.1.3.2 (PR #934). Server `SettingsService.getWhtRates()` already validates optional `effectiveDate` ISO-8601 per entry (delivered in #934). This PR extracts the WHT filter logic into a pure helper `apps/web/src/lib/wht-rates.ts` (`filterActiveWhtRates` + `whtRatesToSelectOptions`); `ItemLinesSection` delegates to it. Future-dated entries hidden client-side; unparseable dates treated as "include" (permissive fallback). 9 new vitest cases (no-date / past-date / boundary equal-to-now / strictly future / mixed / unparseable / select-options shape). 9/9 pass |
| D1.1.3.6 | Admin UI (tax rates tab) | P0 | ✅ | this PR | New OWNER-only route `/settings/tax-rates` → `TaxRatesPage.tsx`. Card-based UI: (1) editable WHT rates table with Add/Edit/Delete actions wired through existing `PATCH /settings` endpoint; (2) SSO rate displayed as locked card (D1.1.3.3 — fixed at 5% by law). Uses shadcn Dialog + Card + Table + Input + Label + ConfirmDialog + sonner toast (no banned APIs). Optimistic update via react-query invalidating `['settings-ui-flags']`. Server-side: new `validateConfigShape()` guard in `SettingsService` rejects malformed `wht_rates` payloads on `update`+`bulkUpdate` (Thai BadRequestException — out-of-range rate / empty label / unparseable effectiveDate / empty array / non-JSON). 6 new settings vitest cases. 39/39 settings spec pass. App.tsx route lazy-loaded |
