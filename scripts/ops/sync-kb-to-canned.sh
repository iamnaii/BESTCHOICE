#!/usr/bin/env bash
# Sync คลังคำตอบบอท (แถวที่เปิดใช้) → ข้อความสำเร็จรูปให้พนักงานกดใน inbox
# รันซ้ำได้ทุกเมื่อหลังแก้คลัง:  bash scripts/ops/sync-kb-to-canned.sh
set -u
cd "$(dirname "$0")/../.."
PORT=${PORT:-15432}
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/sync-kb-to-canned.sql || die "sync พัง"
ok "เสร็จ — พนักงานเห็นในหน้าต่างข้อความสำเร็จรูปทันที"
