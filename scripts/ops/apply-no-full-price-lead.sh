#!/usr/bin/env bash
# v2.19 ห้ามราคาเต็มนำ เทียบด้วยดาวน์+งวด — รัน apply-no-full-price-lead.sql
# — รัน apply-no-full-price-lead.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-no-full-price-lead.sql || die "apply ล้มเหลว"
