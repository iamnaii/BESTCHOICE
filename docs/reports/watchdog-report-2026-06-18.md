...
**5/15 checks passed cleanly — 4 critical failures, 6 warnings.**

Critical path: (1) `@prisma/client-finance` module (SP7.7 entity-split WIP) is not generated, breaking the API TypeScript build (exit 1, 8 errors) and causing 145 API test failures across 14 suites. (2) Deep soft-delete audit found 54 violations across 22 files, including security-critical gaps: deleted payment links accepting slip uploads (TOCTOU), deleted customers completing LINE binding, and deleted SystemConfig rows poisoning the chatbot's live fee calculations.
...
| A4 Soft-Delete | **FAIL** | Deep audit found **54 violations across 22 files** with several security-critical gaps. **Critical (exploitable):** `payment-evidence.service.ts` (7 violations) — deleted payment links accept slip uploads (TOCTOU at line 464: re-check inside `$transaction` does not filter `deletedAt`); deleted evidence can be approved/rejected (lines 125, 199, 246, 332). `verification.service.ts` (3 violations) — soft-deleted customers can complete LINE account binding via OTP (lines 302, 321); deleted `customerLineLink` rows returned as active. `notification-template.service.ts` line 33 — deleted template blocks re-creation of same `eventType` permanently (duplicate-check finds deleted row, throws Conflict). **High (incorrect business logic):** `finance-config.service.ts` lines 48, 109 — deleted `SystemConfig` rows (e.g. overridden `late_fee_per_day`) loaded into bot cache and served as active config. `finance-tools.service.ts` lines 155–156 — deleted late-fee config rows used for customer-facing fine calculations. `crm-pipeline.service.ts` (5 violations) — deleted leads returned and mutated. `room-manager.service.ts` (5 violations) — deleted chat rooms reused, deleted `customerLineLink` used to associate new rooms. **Medium:** `payment-link.service.ts` lines 46, 98 — deleted contracts used to create payment links; deleted links dispatched to customers. `notifications/collections-notifier.service.ts` lines 35, 113 — deleted contracts included in dunning escalations. `notification-reminder.service.ts` line 399 + `owner-report-notifier.service.ts` line 81 — soft-deleted owners with `isActive: true` still receive notifications. **Additional batches (financial/security critical):** `paysolutions-confirmation.service.ts:60` — deleted payment link confirmed, triggering payment processing against a voided link. `paysolutions-webhook.service.ts:125` — webhook matches and credits a soft-deleted payment link (double-credit risk if link was deleted after payment was received). `payment-evidence.service.ts:464` — TOCTOU in `$transaction` allows slip upload against deleted link. `stock-transfer.service.ts:85` — soft-deleted pending transfer phantom-locks product from new transfers. `repossessions.service.ts:515` — deleted `productPrice` row resurrected by update. `mdm/mdm-auto.service.ts:43` — deleted MDM config keys applied to device lock decisions. **Overdue/other-income batch:** `overdue/mdm-lock.service.ts` (5 violations) — deleted MDM lock requests can be approved/rejected/unlocked (lines 106, 176, 198, 380). `overdue/dunning-engine.service.ts:357` — deleted contracts executed in dunning engine. `overdue/contract-letter.service.ts` (3 violations) — deleted contracts/letters processed. `other-income-lifecycle.service.ts` (2 violations) — deleted chart-of-accounts codes used for JE posting. `other-income-config.service.ts` (2 violations) — deleted `SystemConfig` controls maker-checker toggle. Full total: **~100+ violations across 35+ files** — pervasive pattern throughout new modules added post-v4. |
...
3. **Soft-delete: `paysolutions-webhook.service.ts:125` and `paysolutions-confirmation.service.ts:60`** — Both query `paymentLink.findFirst({ where: { token: ... } })` without `deletedAt: null`. A payment link that is soft-deleted (voided) after payment is initiated can still be credited by the PaySolutions webhook — financial double-credit risk. Add `deletedAt: null` to both queries immediately.

4. **Soft-delete: `payment-evidence.service.ts` TOCTOU** — Line 464: `paymentLink.findUnique` inside `$transaction` re-check does not include `deletedAt: null`. A deleted/voided payment link can still accept customer slip uploads. Add `where: { id: link.id, deletedAt: null }` to the re-check. Also fix lines 125, 199, 246, 332 where deleted evidence can be approved/rejected.

4. **Soft-delete: `verification.service.ts`** — Lines 302, 321: `customer.findUnique` and `customerLineLink.findUnique` without `deletedAt: null`. Soft-deleted customer accounts can complete LINE OTP binding. Add `deletedAt: null` filters.

5. **Soft-delete: `notification-template.service.ts:33`** — Duplicate-check query finds deleted templates and permanently blocks re-creation of the same `eventType`. Add `deletedAt: null` to both `findUnique` calls (lines 16, 33). Without this fix, a deleted template locks that event type forever.

6. **Soft-delete: `finance-config.service.ts` + `finance-tools.service.ts`** — Deleted `SystemConfig` rows (e.g. old `late_fee_per_day` value) loaded into bot in-memory cache (line 48) and used for customer-facing fine calculations (lines 155–156). Add `deletedAt: null` filter. Wrong late-fee config = wrong amounts shown to customers.

7. **Soft-delete: `mdm/mdm-auto.service.ts:43`** — Deleted `SystemConfig` keys (`mdm.autoLockEnabled`, `mdm.noAnswerThreshold` etc.) read without `deletedAt: null`. If an MDM config key is soft-deleted (e.g. as part of a config reset), auto-lock/unlock decisions use stale values — devices may fail to lock or unlock incorrectly.

### High (this sprint)

8. **Decimal compliance sweep — critical services first** — Convert `Number()` on `Prisma.Decimal` fields to `new Prisma.Decimal(x)` or `.toNumber()` only at serialization boundaries. Priority files: `chatbot-finance/finance-tools.service.ts` (payment arithmetic), `paysolutions/paysolutions-webhook.service.ts` (payment crediting), `notifications/collections-notifier.service.ts`, `contracts/contract-lifecycle.service.ts`.
