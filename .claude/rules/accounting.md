...
`Σ outstanding` (fee-netted, per `feeNettedOutstanding` above) over each in-scope contract's DUE installments — ties to GL **11-2103** accrued receivable in aggregate. **Late fee is excluded from the base** — it isn't a GL asset (only recognized as `42-1103` income when actually collected), so folding it in would overstate exposure.
...
### TERMINATED contracts — UNIFIED with ACTIVE (carrying-amount base RETIRED 2026-07-26)

v3 gave TERMINATED contracts a special "carrying amount" base (`11-2103 + 11-2101 − 11-2106` GL balances), because the 2A accrual cron stops firing once a contract is TERMINATED and a Payment-rows-based base would otherwise balloon past true carrying value with un-accrued interest/VAT. **v4 retires this override entirely** — `terminatedCarryingAmount()` no longer exists in the code. A TERMINATED contract now ages via the EXACT SAME per-installment engine as an ACTIVE contract: every outstanding installment (DUE selection, independent of contract status) gets its own bucket/rate off its own `Payment.dueDate`.
...
- `reverseStageOnPayment` is likewise unified — no more TERMINATED branch; ACTIVE and TERMINATED contracts alike compute `target` via `computePerInstallmentProvision` and release `min(existing.provisionAmount − target, GL 11-2102)`.
- **The old v3 goldens no longer apply and must not be cited**: the carrying-amount golden (base `12,797.51` → provision `9,598.13`) described a base formula that has been deleted from the code. `ecl-terminated-base.spec.ts` (same file, rewritten) now asserts the v4 per-installment golden instead — see the goldens table below (**2,122.16**).
...
```
Once a contract is repossessed or written off there is no more receivable to provide against, so **11-2102 for that contract always lands on exactly 0** after the JE — via consume alone (provision <= loss), release alone (gain / exact-wash, consume = 0), or both (provision > loss). `metadata.releasedProvision` (string, `"0.00"` when nothing was released) is stamped on every JP5/write-off JE for audit traceability.
...
- `calculateProvisions(...).byBucket` — built by summing each in-scope contract's `bucketAgg` (the same per-bucket aggregation `computePerInstallmentProvision` already computes) into the response, instead of adding the contract's whole `provisionAmount` under its single `contractBucket`. A `{90,60,30}` contract now shows `757.92` on `'61-90'`, `227.37` on `'31-60'`, `30.32` on `'1-30'` — not `1,015.61` dumped entirely onto `'61-90'`.
- `getProvisionSummary().byBucket` — sums each ACTIVE row's persisted `bucketBreakdown` (see Method above) across contracts. Rows persisted BEFORE the per-installment migration carry no `bucketBreakdown` (`null`) — those fall back to the OLD whole-row attribution (their entire outstanding/provision keyed under their single `agingBucket`) so legacy data still reports sensibly instead of silently vanishing from the summary. `rate` per bucket is derived from the aggregated data (`provision / outstanding`) rather than re-read from the live rates config, since a bucket's aggregate reflects whatever rate was actually in effect when each contributing row was calculated. `details[]` keeps its existing shape and additionally passes through each row's `bucketBreakdown` (`null` for legacy rows).
