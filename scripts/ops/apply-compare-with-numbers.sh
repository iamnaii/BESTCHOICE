#!/usr/bin/env bash
# v3.2 เทียบรุ่นด้วยตัวเลขจริง+ส่วนต่าง — รัน apply-compare-with-numbers.sql
# — รัน apply-compare-with-numbers.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-compare-with-numbers.sql || die "apply ล้มเหลว"
