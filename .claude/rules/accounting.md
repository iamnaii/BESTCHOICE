...
- Integration E2E (DB จริง): `apps/api/src/modules/contract-exchange/__tests__/exchange-priced-flow.integration.spec.ts` — Case 2A (21-1106 net 0, Cr 11-2101 = GL-true 11,333.36 ไม่ใช่สูตรคูณ 11,333.28, loss plug 4,126.68, + F2 SHOP legs S11-3001/S11-3002/S41-1101/S41-1201/S50-1101↔S11-2001), ECL 30.32 → 51-1103, cancel วันที่ 15 (reversalJeIds 6: A.1+A.2+A.3+A.4+2 SHOP legs) + วันที่ 45 (reversalJeIds 8: + 2 swept 2A accruals — ทั้งคู่ SUCCEED เหมือนกัน, ไม่มี window/penalty JE อีกต่อไป, owner ยกเลิก 2026-07-31 + mirror-reverse net 0 ทุกบัญชีรวม SHOP + 2A backfill), MEMO (JE count คงเดิม). CI: glob `src/modules/contract-exchange/__tests__/*.integration.spec.ts` ใน deploy-gcp.yml vitest step (jest มองไม่เห็นไฟล์ `*.integration.spec.ts` ตาม testPathIgnorePatterns)

### SHOP-leg wiring บนสัญญาใหม่ (F2, CPA ตอบข้อ 3 = ใช่, 2026-08-02)
...
- **Invariant ตรวจแล้ว**: `downPayment` บนสัญญาใหม่ = 0 เสมอ (hardcode ใน `approvePriced` ทั้ง snapshot branch และ legacy fallback branch) และ `financedAmount` = `sellingPrice` เสมอ (ทั้งสองมาจาก `newPrice` เดียวกัน) → `salePrice` คำนวณเป็น `down+financed` (ไม่ใช้ `contract.sellingPrice` ตรงๆ — กัน legacy fallback branch ที่ clone `old.financedAmount` ซึ่งไม่การันตีเท่ากับ `old.sellingPrice` เมื่อสัญญาเดิมเคยมีดาวน์) → invariant ของ template (`down+financed===salePrice`) ผ่านโดยโครงสร้าง ไม่ใช่บังเอิญ
- **idempotencyKey**: `shop-inventory-transfer:<newContractId>` — รูปแบบเดียวกับที่ `ContractWorkflowService.activate` ใช้ (`contract-workflow.service.ts`) กันโพสต์ซ้ำถ้าสัญญาผ่านสองเส้นทางไหนก็ตาม
...
- **⚠️ ข้อค้นพบสำคัญ (2026-08-02) — ไม่เข้าคิว "จ่ายให้หน้าร้าน (INTER-CO)"**: `ExchangeClearVendor21_1106Template` (A.3) เคลียร์ 21-1101/21-1102 ของสัญญาใหม่**เต็มจำนวนทันที**ในทรานแซคชันเดียวกับ finalize (ใช้ราคารับซื้อผ่าน 21-1106 + ขาเงินสด D5) — ไม่ผ่านรอบจ่าย batch เหมือนสัญญาปกติ. ผลคือ `IntercoPendingService.getPendingContracts()` (`HAVING SUM(credit-debit) > 0` บน 21-1101+21-1102) จะ**ไม่เจอสัญญาเปลี่ยนเครื่องเลย** ไม่ว่าจะ wire ฝั่ง SHOP หรือไม่ — ไม่ใช่ "legacyNoShop=true" แต่คือ**ไม่โผล่ในคิวเลย**. F2 แก้ปัญหาสมุด SHOP ไม่มีรายได้/ลูกหนี้ได้ตรงจุด แต่**เปิดช่องว่างใหม่**: S11-3001/S11-3002 ที่ตั้งขึ้นไม่มี JE ฝั่ง SHOP ที่ล้างบัญชี (ไม่มี "รับเงิน/เครดิต" คู่กับขาเงินสดที่ A.3 บันทึกฝั่ง FINANCE) — ยังไม่ได้สร้าง JE ล้างบัญชีตัวนี้เอง เพราะเป็นการออกแบบ JE เคลื่อนเงินใหม่ที่ยังไม่มี CPA/owner sign-off ดูรายละเอียดที่ `docs/superpowers/specs/2026-07-30-interco-settlement-batch-design.md` §11 ข้อ 2
- **Coverage**: ทดสอบครบใน integration spec ด้านบน (booking values + cancel-sweep + net-zero ทุกบัญชี SHOP) — ยังไม่มี unit-level golden spec แยกต่างหาก (ใช้ `ShopInventoryTransferTemplate`'s เดิมที่มี golden spec อยู่แล้วที่ `apps/api/src/modules/journal/shop-templates/shop-inventory-transfer.template.spec.ts`)
