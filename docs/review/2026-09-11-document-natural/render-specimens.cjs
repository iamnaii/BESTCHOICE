...
(async()=>{
 ...
 for(const long of [false,true]) {
  ...
  if(long) fs.writeFileSync(path.join(out,'e-tax-continuation.pdf'),new ETaxService(null).buildInvoicePdf({payment:{...payment,contract:{...payment.contract,customer:{...payment.contract.customer,addressIdCard:company.address.repeat(18)}}},issuer:{...co,address:company.address.repeat(18)},base:D(1000),vat:D(70),total:D(1070)}));
