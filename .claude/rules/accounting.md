...
- ~~Pro-rate CN for the `PARTIALLY_PAID`/HELD case~~ — DONE 2026-07-26 (CPA ruling): `computeCnBreakdown` pro-rates every accrued-unpaid installment's CN VAT to its outstanding balance; `CreditNoteDocumentService` auto-issues for every case, no more HELD state (see "Pro-rate — HELD gate retired" above). The manual CN-issue endpoint (for a JE that posted before this document-issuance step existed, or before this pro-rate util existed) is a separate follow-up — CN pro-rate plan Task 5.
