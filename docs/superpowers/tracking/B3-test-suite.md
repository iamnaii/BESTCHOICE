...
| B3.K-04 | ภ.พ.30 export uses `11-4101` balance for input VAT refund | P2 | ✅ | this PR | **Implementation gap closed.** `tax.service.ts:previewPP30` previously returned `totalVatInput = 0` (TODO: "ExpenseDocument-based tax reporting will be added in a follow-up PR"). This PR adds `getInputVatLineItems(branchIds, startDate, endDate)` private helper that queries `journal_lines` where `account_code='11-4101'` + `debit > 0` + `metadata.flow LIKE 'expense-%'`, joins `expense_documents` for vendor info, scoped by company branches. Credit-note reversal lines (Cr 11-4101) excluded via debit-positive filter so the period nets correctly. Anti-regression: query is hard-coded to `11-4101`, never `11-2104` (ม.83/6 overseas). 6 new tests in [tax.service.spec.ts](../../../apps/api/src/modules/tax/__tests__/tax.service.spec.ts). |
