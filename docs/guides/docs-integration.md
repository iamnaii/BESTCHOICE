...
5. **ตรวจ PDF ทุกไฟล์ที่รอบนั้นสร้าง** (`tools/docs-integration.mjs verify` — DOC-11): parse ด้วย pdfjs ตัวเดียวกับ spec แล้ว fail ถ้า parse ไม่ได้, หน้าไม่ใช่ A4 / A4 แนวนอน / สติกเกอร์ 50×30 mm (±1.5 pt), มีฟอนต์อื่นนอกจาก THSarabunPSK บนหน้า A4 (สติกเกอร์ 50×30 mm ใช้ฟอนต์ของตัวเองตามคำตัดสินเจ้าของ 2026-09-12 — บันทึกฟอนต์ไว้แต่ไม่บังคับ), มีข้อความนอกขอบหน้า (±2 pt) หรือมีหน้าที่ไม่มีข้อความเลย (render เปล่า) → `artifact-checks.json`; map เอกสารทั้ง 12 กลุ่ม (DOC-01…10 + สัญญา/PDPA + สติกเกอร์) กับ scenario ที่ผ่าน → `manifest.json.coverage`; เขียน `summary.md` (ความล้มเหลว, coverage, ตาราง domain, ผลตรวจไฟล์, ชั้นหลักฐาน) — สถานะ PASS ต้องผ่านทั้ง jest, scenario, และการตรวจไฟล์
...
- `receiving-fixtures.ts` (DOC-02): `createSupplier()` (ผู้จำหน่ายนิติบุคคลมี marker, เบอร์โทรบังคับ) · `markMainWarehouse()` (ตั้งสาขา A เป็นคลังกลางที่ใบรับของนำเครื่องเข้า) · `attachBranchToShop()` (ผูกสาขากับ SHOP + `shopCashAccountCode` S11-1102 ที่การรับซื้อเงินสดต้องมี — fail-closed) · `seedTradeInValuation()` (แถวราคากลางจำลองสำหรับด่าน ±15%) · `checksummedNationalId()` (เลขบัตร 13 หลัก checksum ถูกต้อง ช่วง 7xxxx สังเคราะห์) · `imeiFactory()` (IMEI 15 หลักไม่ซ้ำต่อ run) · `receivingExpectation()` (PASS → สินค้า, REJECT → ไม่มีสินค้า) · `expectedBuybackJournal()` / `expectedCreditJournal()` (Dr S11-2002 / Cr เงินสดสาขา|S11-1202 หรือ Cr S21-2003) · `dispositionFilename()` (ถอด `filename*=UTF-8''`) · regex `PO_NUMBER`/`GR_NUMBER`/`VOUCHER_NUMBER`
...
| สติกเกอร์ 50×30 mm (regression, DOC-11) | `pages/StickerPrintPage/` (พิมพ์ผ่านเบราว์เซอร์ `@page { size: 50mm 30mm }`), `modules/stickers` | `stickers.browser.docs-spec.ts`: หน้า `/stickers?productIds=` → print media → `page.pdf({ preferCSSPageSize })` → หนึ่งหน้า 141.73 × 85.04 pt ต่อสติกเกอร์, ฟอนต์บันทึกไว้ (ดวงคงฟอนต์ของ #1575 = IBM Plex Sans Thai ตามคำตัดสินเจ้าของ 2026-09-12), รุ่นเต็มบนทุกใบ (บรรทัดรุ่นเป็น nowrap+ellipsis จึงใช้ชื่อรุ่นจริง), ไม่มีข้อความนอกหน้า — ไม่ assert ราคา/ถ้อยคำ; หน้าอยู่ที่ `pages/StickerPrintPage/` ตั้งแต่ #1575 |
