...
- This command's default preview covers Inbox, customers, credit, offers, contract handoff, dashboard reads, the FINANCE portfolio, and trade-in purchase through product photos/pricing/stock entry with synthetic data/AI. For other features, also run the appropriate app/API locally from this checkout and exercise the changed flow; do not claim this limited preview validates the whole application.
