#!/usr/bin/env bash
# v3.9 ตัดคำชวนจองทุกจุด (คำสั่งเจ้าของ) — รัน apply-no-booking-v3-9.sql
# — รัน apply-no-booking-v3-9.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-no-booking-v3-9.sql || die "apply ล้มเหลว"
