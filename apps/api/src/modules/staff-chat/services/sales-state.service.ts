...
export interface SalesState {
  ...
  paymentMode?: 'cash' | 'installment' | null; // วิธีซื้อที่ลูกค้าเลือก — ห้ามถามซ้ำ ห้ามสลับเอง
...
export class SalesStateService {
  ...
  buildNote(state: SalesState | null): string | null {
    ...
    if (state.paymentMode)
      lines.push(`วิธีซื้อที่ลูกค้าเลือก: ${state.paymentMode === 'cash' ? 'เงินสด' : 'ผ่อน'}`);
