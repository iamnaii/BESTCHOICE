...
- **แท็บ "รอตอบ"** ใน `ChannelFilter` (`InboxTab = 'waiting' | 'mine' | 'all'` — ไม่มีไอคอน ไม่มี "ยังไม่อ่าน" ดูแก้ไข 2026-09-05 ด้านล่าง) วางเป็นแท็บแรก · `index.tsx` ค่าเริ่มต้น `tab: 'waiting'` · ส่ง `waiting: true` เป็น query param · ตัวเลขจาก `serverCounts.waiting`

> **แก้ไข 2026-09-05 (วัด prod: คิว 68 ห้อง ตอบได้จริง 24 · ห้องแรกที่ตอบได้อยู่ลำดับ 42 เมื่อเรียงรอนานสุดก่อน)**
> - **"รอตอบ" = รอตอบ *และยังตอบทัน*** — ห้อง FACEBOOK ที่ `lastCustomerAt` เก่ากว่า 24 ชม. (หรือยังไม่มีค่า) **ไม่อยู่ในแท็บนี้** ไปอยู่มุมมอง **"ตอบไม่ทัน"** (`expired: true`) ที่หัวหน้าเปิดดูเพื่อติดต่อทางอื่น · ช่องทางที่ไม่มีหน้าต่าง (LINE) อยู่ในรอตอบเสมอ
> - **เรียงสองชั้น "ใกล้หมดเวลาก่อน แล้วรอนานก่อน"**: ชั้น 1 = FACEBOOK ที่เหลือ ≤ 3 ชม. เรียง `lastCustomerAt asc` · ชั้น 2 = ที่เหลือทุกช่องทาง เรียง `waitingSince asc` — ไม่ใช่ `waitingSince asc` ล้วน (ห้อง LINE ที่รอ 3 วันต้องไม่จมใต้ห้อง FB ที่เพิ่งทัก และห้อง FB ที่ลูกค้าทักซ้ำเมื่อ 2 ชม. ต้องไม่แซงห้องที่เหลือ 4 ชม.)
> - คอลัมน์ใหม่ `ChatRoom.lastCustomerAt` ตั้งใน `saveMessage` สาขา CUSTOMER แบบ**เดินหน้าอย่างเดียว** (ไม่ใช่ set-if-null) · CLI เติมย้อนหลังจาก `max(created_at)` ของข้อความ CUSTOMER
> - แท็บ "ยังไม่อ่าน" ถูกตัด (วัดว่ามีคนกดเปิดหรือยัง ไม่ใช่ข้อเท็จจริงฝั่งลูกค้า) · ไอคอนบนแท็บถูกตัด (4 ใบ+ไอคอน = 342px ล้นที่ว่าง 287px) · เลขบนแท็บ "ทั้งหมด" ถูกตัด (99+ ตลอดกาล)
> - ตัวกรองแถวสอง = **เมนูช่องทาง** (เลือกทีละอัน "ทุกช่องทาง" = ล้าง · เจ้าของเคาะ) + **เมนูผู้ดูแล** (ทุกคน / ยังไม่มีคนดูแล / รายชื่อ) แทนเมนูสถานะบอท (บอทตอบ 3/8,386 ห้อง = ปุ่มตาย) · ทั้งสองกรองเฉพาะรายการ ไม่แตะเลขบนแท็บ
> - ป้ายบนแถวไม่เกิน 2 ใบ + "+N" ลำดับ: ป้ายหน้าต่าง (เหลือ N / หมดเวลาตอบ / รอ N) → ด่วน → ค้างชำระ → บอทตอบ
...
> **แก้ไข 2026-09-05:** "บนลงล่าง" ใช้ได้ก็ต่อเมื่อแท็บรอตอบมีเฉพาะห้องที่ยังตอบทันและเรียงสองชั้นตาม §7 — ก่อนแก้ ห้องแรกที่ตอบได้อยู่ลำดับ 42/68 · และเสียงเตือนต้องดังจริง: `getWsBaseUrl()` ใน `useChatSocket.ts` ต้องใช้ `window.location.origin` เมื่อ API เป็น path สัมพัทธ์ (เดิมตกไป `localhost:3000` บน prod ⇒ ไม่เคยดัง) · ห้องที่ "ตอบไม่ทัน" ให้หัวหน้าเปิดมุมมองนั้นแล้วโทรตาม
