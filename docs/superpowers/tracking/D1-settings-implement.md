...
**Started:** 2026-05-16  |  **PRs:** #882-#900 · this PR (D1.1.6.3)  |  **Done:** 20/75 — 2.6 ✅ · 2.7 ✅ · 2.2 ✅ · 1.6 ✅ Complete (3/3)
...
| D1.1.6.1 | `adj_underpay_account` (wire consumers to AccountRoleService) | P0 | ✅ | this PR | Injected `AccountRoleService` into `PaymentReceipt2BTemplate` (constructor arg 3, before `@Optional() vat60Reversal`), `PaymentReceipt2BSplitTemplate` (constructor arg 3), and `PaymentsService` (constructor — between `badDebtService` and `@Optional() mdmAuto`). All 3 hardcoded `'52-1104'` literals replaced with `roles.code('adj_underpay')` (rounding underpay branch in full 2B, partial-final underpay branch in 2B-split, preview rounding branch in PaymentsService.previewPaymentJE). Seed already provides `adj_underpay → 52-1104` row in `account_role_map`. Specs updated: `payment-receipt-2b.template.spec.ts` (15 sites + new D1.1.6.1 override test), `payment-receipt-2b-split.template.spec.ts` (2 sites + new D1.1.6.1 override test), `receipt-void-reversal.template.spec.ts` (1 site), `reschedule-jp6.template.spec.ts` (3 sites), `early-payoff-jp4.template.spec.ts` (2 sites), `repossession-jp5.template.spec.ts` (1 site), `payments.service.spec.ts`, `payments.service.advance.spec.ts`, `payments-financial.integration.spec.ts` — provide mocked AccountRoleService. **AdjustmentSection.tsx:30 NOT wired** — that's a UI suggestion hint to the user; the JE-posting source of truth is now the role map. Frontend wiring can come in D1.1.6.2 along with the overpay hint. TypeScript: 0 errors. Jest: 54+23 tests pass (payments.service / advance). Vitest cpa-template specs fail locally on pre-existing `chart_of_accounts.createdAt` schema drift (memory: "Local dev DB in db push state") — CI passes against migrated DB |
| D1.1.6.2 | `adj_overpay_account` (wire consumers to AccountRoleService) | P0 | ✅ | this PR | Backend: 3 hardcoded `'53-1503'` literals replaced with `roles.code('adj_overpay')` (overpay branch in 2B, 2B-split, and PaymentsService.previewJournal). Frontend: new `GET /chart-of-accounts/adjustment-roles` endpoint + `useAdjustmentRoleCodes()` hook + `AdjustmentSection.tsx` builds DEFAULT_SUGGESTED from API (falls back to seeded `53-1503`/`52-1104` during first paint). JSDoc updated on both templates ("Cr <adj_overpay role>… 53-1503 by default"). 1 new jest test on `ChartOfAccountsService.getAdjustmentRoleCodes`. Stacks on #899 (D1.1.6.1). Expense module `ADJUSTMENT_ALLOWLIST` deferred to follow-up — needs holistic refactor (also covers `52-1106`/`53-1303` which are not role-driven) |
| D1.1.6.3 | `adj_auto_route` toggle | P0 | ✅ | this PR | SystemConfig key `adj_auto_route` (default `true`). New `AccountRoleService.isAdjustmentAutoRouteEnabled()` reads the flag, treats only `false`/`0`/`off` as disabled, falls through to `true` on any failure. Guards in 3 sites: `PaymentReceipt2BTemplate.execute` (throws BadRequestException with Thai message when diff≠0), `PaymentReceipt2BSplitTemplate.executePartial` (same), `PaymentsService.previewJournal` (omits rounding line → `isBalanced=false` so the UI surfaces manual-adjustment path). 1 new D1.1.6.3 jest test in `payments.service.spec.ts` proves preview goes unbalanced. Stacks on #900 → completes 1.6 sub-section (3/3) |
