...
### คำตัดสินเจ้าของ — "บันทึกขายผิด" (ปิดประเด็นแล้ว 2026-08-22)

**"บันทึกขายผิด" วันนี้ไม่มีทางแก้ในระบบเลย** (final review I-2, ตรวจแล้ว 2026-08-22):

- `FOUND_POLICY` ปิด `SOLD_CASH`/`SOLD_RESELL` — **ถูกต้องแล้ว**: ทางเดิมพาเครื่องกลับเข้า
  `IN_STOCK` โดยทิ้งแถว `Sale` ไว้ = ขายซ้ำเครื่องเดียวกันได้
- `SalesService` มีแค่ `findAll` / `findOne` / `create` (controller ไม่มี `@Patch`/`@Delete`
  และ `@Post` ตัวเดียวคือสร้างการขาย) และ `SOLD_*` อยู่ใน `SYSTEM_MANAGED_STATUSES` ⇒
  PATCH สถานะสินค้าก็ปิด ⇒ **ไม่เหลือเมนูใดเลย**
- ข้อความบนด่านซื่อสัตย์ว่า "ระบบยังไม่มีเมนูยกเลิกการขาย" แต่ remedy ที่มันชี้
  ("ให้เจ้าของ/ฝ่ายบัญชีแก้รายการขายใบนั้นก่อน") **แปลว่าแก้ DB ด้วยมือ** — เป็น pattern
  "ชี้ทางที่ไม่มีจริง" ตัวเดียวกับที่โดนจับ 3 รอบใน Tasks 2-3 เพียงแต่รอบนี้แก้ที่ข้อความ
  ไม่ได้ เพราะทางที่ถูกต้องยังไม่มีอยู่จริง

**คำตัดสินเจ้าของ 2026-08-22: (ก) ยอมรับช่องว่างนี้ไว้ก่อน** — บันทึกขายผิดให้แก้ที่ฐานข้อมูล
เป็นราย ๆ โดยผู้ดูแลระบบ, ไม่สร้าง flow "ยกเลิกใบขาย" ในตอนนี้ (เจ้าของประเมินว่าเกิดไม่บ่อย
พอที่จะคุ้มกับการทำทั้งชุด). **อย่าเสนอซ้ำ** จนกว่าจะมีเคสจริงบ่อยพอ — ถ้าถึงตอนนั้น ตัวเลือก (ข)
คือทำ flow "ยกเลิกใบขาย" จริง — ซึ่งต้องออกแบบครบชุด: กลับสถานะสินค้า +
กลับรายการ `ShopCashSaleTemplate` (SHOP ไม่จด VAT จึงไม่มีใบลดหนี้ภาษี แต่ต้องมีเอกสาร
อ้างอิง) + 4-eyes + audit + กันยกเลิกใบที่ข้ามงวดบัญชีปิดแล้ว. **ห้ามเดาแล้วสร้างเอง** —
คลาสเดียวกับ opening-balance gap (interco spec §11) ที่ต้องรอคำตัดสิน
