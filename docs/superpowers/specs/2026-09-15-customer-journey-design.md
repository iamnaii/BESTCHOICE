...
- ใครในทีมตอบแชทใน Facebook app (echo staff_id ≈0) และข้อความทักทายอัตโนมัติ FB ถูกเก็บเป็น STAFF — แก้ด้วยโค้ดไม่ได้ · ทางที่เล็กสุดคือแสดงเป็น 'ร้าน (ไม่ทราบชื่อ)' ติดป้ายค่าประมาณ ข้ามข้อความทักทายตามกติกา shouldSkipFirstOutboundClear และไม่ใช้เป็นขั้นของ funnel · แก้จริงได้เมื่อทีมย้ายมาตอบในอินบ็อกซ์
  - งานต่อ (ยังไม่ทำ): เก็บ `message.app_id` ของ echo ลงคอลัมน์ใหม่แบบ additive nullable บน chat_messages (webhook log แล้วแต่ไม่เก็บ) + grant ให้ mcp_ro · ก่อนเพิ่มคอลัมน์ให้วัดการกระจายของ app_id จาก Cloud Logging (บรรทัด `[FB Webhook] Echo …` — นับอย่างเดียว บรรทัดนั้นมี PSID) ว่าแยกข้อความอัตโนมัติของเพจออกจากคนตอบได้จริงไหม ถ้าได้ ใช้แทนกติกาเวลา anchor ของ FIRST_STAFF_REPLY ได้
