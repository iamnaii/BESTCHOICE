#!/usr/bin/env bash
# ขยายเพดานตอบ 50→300 ช่วงเทส — รัน raise-auto-reply-cap.sql
# — รัน raise-auto-reply-cap.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/raise-auto-reply-cap.sql || die "apply ล้มเหลว"
