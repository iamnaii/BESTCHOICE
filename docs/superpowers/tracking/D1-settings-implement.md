...
| D1.3.5.3 | `summary_pagination_size` | P2 | ✅ | this PR | SystemConfig `summary_pagination_size` (default 100, clamped 10-500 on read). `getUiFlags()` adds `summaryPaginationSize`. `ExpenseDailySummaryPage` slices the docs table to `visibleCount = pageSize × (1 + extraPages)` with a "แสดงเพิ่มอีก N รายการ" expander. Grand total comes from server (`summary.grandTotal`) so pagination never under-reports. Will be a SUMMARY-SPECIFIC override of generic D1.2.3.2 `paginationSize` once that ships. 4 jest cases (default / valid 10-500 / above 500 → clamp / below 10 → clamp) |
