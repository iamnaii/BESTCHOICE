  ...
  it('สร้างสัญญา: สินค้าถูกลบแทรกหลังด่านนอก tx → ด่านใน tx ต้องจับได้ (Important 3)', async () => {
    ...
    try {
      await expect(
        ...
        // ตั้งแต่ 17f19230f create() คำนวณ quote (ContractQuoteService) ก่อนด่านสต็อกใน tx —
        // เครื่องที่ถูกลบระหว่างด่านจึงถูกปฏิเสธที่ quote ('ไม่พบสินค้าที่ต้องการคำนวณ') หรือที่ด่านสต็อก
        // ('สินค้าไม่พร้อมขาย') แล้วแต่จังหวะ; สิ่งที่ต้องจริงคือถูกปฏิเสธหลังการแทรก และไม่มีสัญญาเกิด
      ).rejects.toThrow(/สินค้าไม่พร้อมขาย|ไม่พบสินค้าที่ต้องการคำนวณ/);
