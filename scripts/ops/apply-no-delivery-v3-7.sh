#!/usr/bin/env bash
# v3.7 ไม่มีบริการจัดส่ง + marker รูปฝั่งลูกค้า + KB รับที่ร้าน — รัน apply-no-delivery-v3-7.sql
# — รัน apply-no-delivery-v3-7.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-no-delivery-v3-7.sql || die "apply ล้มเหลว"
