...
| D1.1.2.1 | `doc_prefix_per_type` | P0 | ✅ | this PR | Q3=accept current. SystemConfig key `doc_prefix_per_type` (JSON map DocumentType→prefix). `SettingsService.getDocPrefixMap()` returns the parsed map, falls back to `DEFAULT_DOC_PREFIX_MAP` when missing/malformed. `DocNumberService.next()` resolves via the map (was `PREFIX_MAP` constant). `getUiFlags()` exposes `docPrefixMap` for UI rendering. Type-check 0 errors |
