...
export function tradeInSellerEvidenceError(e: TradeInEvidence): string | null {
  ...
  if (!/^\d{13}$/.test(id) || (11 - (sum % 11)) % 10 !== Number(id[12]))
    return 'กรุณาระบุเลขบัตรประชาชนผู้ขายให้ถูกต้อง';
...
export function tradeInEvidenceError(e: TradeInEvidence): string | null {
  ...
  return tradeInDeviceEvidenceError(e);
}

/** Device validation can run before collecting the seller's personal information. */
export function tradeInDeviceEvidenceError(e: TradeInEvidence): string | null {
  if (!e.imei?.trim() && !e.serialNumber?.trim())
    return 'ต้องมี IMEI หรือ Serial Number อย่างน้อยหนึ่งรายการ';
  ...
  if (!e.serialNumber?.trim() && !e.serialNumberMissingReason?.trim())
    return 'กรุณาระบุเหตุผลที่ไม่มี Serial Number';
  if ((e.imeiMissingReason?.length ?? 0) > 300 || (e.serialNumberMissingReason?.length ?? 0) > 300)
    return 'เหตุผลต้องไม่เกิน 300 ตัวอักษร';
  return null;
}
