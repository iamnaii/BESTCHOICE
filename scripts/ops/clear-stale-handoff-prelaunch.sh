#!/usr/bin/env bash
# (ดูหัวไฟล์ .sql) — รัน clear-stale-handoff-prelaunch.sql
# — รัน clear-stale-handoff-prelaunch.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/clear-stale-handoff-prelaunch.sql || die "apply ล้มเหลว"
