#!/usr/bin/env bash
# v2.18 เรียก tool พร้อมกันในรอบเดียว — รัน apply-parallel-tool-calls.sql
# — รัน apply-parallel-tool-calls.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-parallel-tool-calls.sql || die "apply ล้มเหลว"
