...
- Task 11 (bump version · e2e smoke · ตรวจทั้งชุด): merge `origin/main` (after-sales hub PR 2, web 26.9.53) เข้า `feat/gfin-precheck` แบบ merge commit (`b9bddc7d8`) — 0 conflict, `prisma validate`/`generate` ผ่าน, migration ทั้งสองฝั่งเรียงถูกลำดับ · bump web → `26.9.54` · เพิ่ม `apps/web/e2e/gfin-precheck.spec.ts` (แก้ 2 จุดจากบรีฟต้นฉบับให้รันผ่านจริง: `page.request.get` ต้องส่ง header เอง — `page.setExtraHTTPHeaders` ไม่ไหลไปที่ `page.request` ซึ่งเป็นคนละ channel กัน ใช้ `getAuthHeaders()` ที่มีอยู่แล้ว ตามธรรมเนียมทุกสเปกอื่นในโปรเจกต์; และ endpoint รายการห้องแชทห่อ envelope สองชั้น `{success,data:{data,total,page,limit}}` ต้องแกะสองรอบ) — รันผ่าน chromium (firefox/webkit ไม่มี browser binary ติดตั้งในเครื่องนี้ — CI จริงก็ติดตั้งแค่ chromium `--with-deps chromium` เหมือนกัน ไม่ใช่ gap ของงานนี้) · ตรวจทั้งชุด: type-check 0 error, API jest 9705/9706 ผ่าน (เทสต์ 1 ตัวที่ล้มรอบแรกเกิดจากห้องแชททดสอบที่เราสร้างเองไปกวนเทสต์ `backfill-chat-prospects.db.spec.ts` ซึ่งนับจำนวนห้องทั้งตารางแบบไม่ scope — ลบห้องทดสอบแล้วรันซ้ำผ่าน 100%, ไม่ใช่ regression จากโค้ด PR นี้), web vitest 2682/2682 + lint 0 error (507 warning เดิม), packages/shared vitest 111/111
