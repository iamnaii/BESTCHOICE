...
import { DEFAULT_DOC_PREFIX_MAP, DOC_PREFIX_REGEX } from '../settings.constants';

/**
 * Read-mostly SystemConfig flag accessors decomposed out of the monolithic
 * SettingsService (Wave-4). This is the dependency ROOT for the settings
 * cluster — `getKey` (made public here) is consumed by DocNumberPreviewService.
 * All method bodies are byte-identical to the original; only `this.prisma`
 * field resolution + import paths changed.
 */
@Injectable()
export class SettingsFlagsService {
