#!/usr/bin/env bash
# ล้างธง handoff ห้องทดสอบ whitelist — รัน clear-test-room-handoff.sql
# — รัน clear-test-room-handoff.sql (เขียนทับทั้งก้อนแบบ md5-guard)
# รัน:  bash scripts/ops/clear-test-room-handoff.sh
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/clear-test-room-handoff.sql || die "apply ล้มเหลว"
