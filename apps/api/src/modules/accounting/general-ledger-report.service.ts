 ...
 * AccountingService. These pull from the JournalEntry/JournalLine general
 * ledger (Phase A.4 CPA chart) — Trial Balance, journal P&L, journal Balance
 * Sheet, Cash Flow (indirect), Equity Statement, General Ledger, and the
 * paginated General Journal. They are distinct from the transactional
 * (sales/payment-aggregate) reports in TransactionalReportService.
 *
 * Behavior-preserving move; logic is verbatim from the original
 * AccountingService methods. AccountingService now delegates to this service.
 *
 * One cohesive service: the methods call each other (getBalanceSheetFromJournal
 * → getTrialBalance; cash-flow/equity/ledger → getProfitLossFromJournal +
 * private sumAccountBalances / sumDebitInPeriod) so splitting would force
 * cross-service helper sharing.
 */
@Injectable()
