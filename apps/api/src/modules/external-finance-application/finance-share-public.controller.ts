...
export class FinanceSharePublicController {
  ...
  constructor(private share: FinanceShareService, private config: ConfigService, private lineGroup: GfinLineGroupService) {}
  ...
  async page(@Param('token') token: string, @Req() req: Request, @Res() res: Response) {
    ...
    try {
      ...
      return res.status(200).send(buildFinanceSharePage({
        ...
        fileCount: groups.reduce((n, g) => n + g.files.length, 0), lineGroupName,
      }));
    } catch (err) {
