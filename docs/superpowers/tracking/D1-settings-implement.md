...
| D1.1.6.1 | `adj_underpay_account` (wire consumers to AccountRoleService) | P0 | ✅ | this PR | Added `adj_underpay` to `AccountRoleService.REQUIRED_ROLES` (boot-guarded). Three consumers now resolve via `this.roles?.tryCode('adj_underpay') ?? '52-1104'` with `@Optional()` DI: `PaymentReceipt2BTemplate` (underpay rounding line), `PaymentReceipt2BSplitTemplate` (final-partial underpay), `PaymentsService` JE-preview path. 3 new jest tests on AccountRoleService boot guard (boots / throws when missing / tryCode fallback). Owner can remap 52-1104 → any code via admin UI without redeploying templates |
