...
**Trigger.** `RepossessionsService.create` (JP5) and `BadDebtService.writeOffBadDebt` both call `CreditNoteDocumentService.issueForContract` — gated on the JE's own `metadata.creditNoteIssued === true` (i.e. `RepossessionJP5Template`/`BadDebtWriteOffTemplate` found ≥1 accrued-unpaid installment) — **inside the same `$transaction`** that posted the source JE, so a CN-issuance failure rolls back the JE too (atomic). LINE delivery (`CreditNoteDeliveryService.deliver`) is deliberately NOT called from inside that tx — both callers fire it fire-and-forget **after** the tx commits (`void this.cnDeliveryService.deliver(receiptId).catch(Sentry.captureException)`), so a rollback can never hand the customer a link to a receipt that turned out not to exist.
...
**Follow-ups pending (documented, not yet done):**
- Pro-rate CN for the `PARTIALLY_PAID`/HELD case — needs CPA ruling.
