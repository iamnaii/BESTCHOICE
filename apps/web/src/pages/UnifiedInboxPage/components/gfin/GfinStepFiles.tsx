...
import { GFIN_ACCEPT, PRIMARY_SLOTS, REQUIRED_SLOTS, SLOT_LABELS, SLOT_ORDER, quietly, slotCounts, type FinanceSlot } from './gfin';
...
export default function GfinStepFiles({ gfin, onBack, onNext, dropFiles, onDropFilesHandled }: { gfin: FinanceApplicationModel; onBack?: () => void; onNext?: () => void; dropFiles: File[] | null; onDropFilesHandled: () => void }) {
  ...
  return (
    <Group label="3 รูป" count={app.files.length} right={<button type="button" onClick={() => setPendingFiles([])}>อัปโหลด</button>}>
          ...
          <li key={slot} className="py-1.5 text-xs leading-snug">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex shrink-0 gap-1">
                ...
                {slot === 'DEVICE_PHOTO' && counts[slot] === 0 && usedPhone && <Button size="sm" variant="outline" disabled={gfin.busy} onClick={() => quietly(gfin.fromProduct())}>ดึงจากสต๊อก</Button>}
      ...
      <input ref={input} type="file" multiple accept={GFIN_ACCEPT} capture={uploadSlot === 'DEVICE_SCREEN' ? 'environment' : undefined} className="hidden" onChange={e => { const files = Array.from(e.target.files ?? []); e.target.value = ''; if (files.length && uploadSlot) quietly(gfin.upload(uploadSlot, files)); setUploadSlot(null); }} />
      <GfinSlotPicker open={pendingFiles !== null} onOpenChange={o => !o && setPendingFiles(null)} counts={counts} title={pendingFiles?.length ? `ไฟล์ ${pendingFiles.length} ไฟล์ ใส่ช่องไหน?` : 'อัปโหลดเข้าช่องไหน?'} onPick={slot => { const files = pendingFiles ?? []; setPendingFiles(null); if (files.length) quietly(gfin.upload(slot, files)); else setUploadSlot(slot); }} />
