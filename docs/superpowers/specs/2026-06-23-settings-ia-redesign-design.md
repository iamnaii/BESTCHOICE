...
| URL | **ใหม่ตามหมวด** `/settings/<categoryId>[/<itemId>]` + redirect ของเก่า |
| Sidebar | **ยุบ settings zone เหลือ "ตั้งค่าระบบ" อันเดียว** |
...
interface SettingsItem {
  ...
  // inline: render component ในหน้าหมวด | route: หน้าเต็มใต้ layout | external: ลิงก์ออกไป path เดิม (operational)
...
`kind`: **I**=inline section · **R**=route page ใต้ /settings · **X**=external (operational, คง path เดิม, โชว์ใน panel เป็นลิงก์ออก)
...
| 3 accounting | เลขที่/รูปแบบเอกสาร | R | /settings/accounting/documents | /settings/document-config (+ลบ dup) |
...
| 7 ai | AI Admin/Persona/Assistant/Training/Performance | R | /settings/ai/{admin,persona,assistant,training,performance} | /settings/ai-* |
...
- ยุบ section "ตั้งค่า" + "AI" (เมนู) เหลือ **"ตั้งค่าระบบ" → /settings** อันเดียว
- operational (ผู้ใช้/สาขา/โปรโมชัน/แบบสัญญา) ไม่อยู่ใน sidebar settings zone แล้ว — เข้าผ่าน panel + ค้นหา + CommandPalette (มี /users อยู่แล้ว — เพิ่มที่เหลือ)
...
- **P3 — Sidebar collapse + operational links + CommandPalette**
...
- ความเสี่ยงหลัก: P2 แตะ ~22 หน้า + redirects เยอะ → ต้องทำทีละหมวด + เทส redirect ทุกตัว
```
