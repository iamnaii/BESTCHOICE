...
- A BUYBACK accept (CASH) posts one balanced JE Dr S11-2002 / Cr branch-till for the buy price; TRANSFER routes Cr to S11-1202. An EXCHANGE accept posts no SHOP JE.
- (Note: `accept()` itself guards `status === 'APPRAISED'` and flips status to `ACCEPTED`, so it cannot be called twice — the template's `shop-trade-in:<id>` idempotency is defense-in-depth, not a reachable re-accept path; don't write a "re-accept" test.)

## Out of scope / deferred
- **EXCHANGE trade-ins** — the trade-in value is a credit toward a purchase; its SHOP accounting must be threaded into the companion sale/contract JE, not a standalone cash-out. **Verified there is no existing linkage** to thread into: no code in `modules/sales` or `modules/contracts` references a trade-in, and `Sale` has no trade-in relation — so the EXCHANGE trade-in value is currently unmodeled in the ledger. Deferring is therefore correct (nothing to post against yet), but note the **known gap**: an EXCHANGE accept still runs `product.create` (device enters physical stock) while posting **no** `S11-2002` entry, so the SHOP used-inventory ledger will **under-count vs physical stock for every EXCHANGE trade-in** until EXCHANGE is modeled. Acceptable for now — `/shop/accounting` carries the not-authoritative disclaimer — but track it as the EXCHANGE follow-up (model the trade-in→sale credit + book S11-2002 on the EXCHANGE leg).
...
- **⚠️ KNOWN GAP — walk-in / staff / quickBuy BUYBACKs are NOT booked (flow-attribution upstream).** `TradeIn.flow` defaults to `EXCHANGE` (`schema.prisma:4521`) and the staff `create()` / `quickBuy()` paths + their DTOs never set `flow` — only the **online web-shop buyback intake** (`shop-buyback.service.ts` → `trade-in-intake.service.ts`) sets `flow=BUYBACK`. So this JE fires only for online buybacks; **in-store cash buybacks (the dominant business case) are tagged `EXCHANGE` and skip the JE**, leaving S11-2002 under-counted vs physical stock. The P3 wiring itself is correct and never produces a half-posted state (it posts nothing for `EXCHANGE`); the gap is upstream flow attribution. **"P3 ships BUYBACK JE wiring" ≠ "all cash buybacks are booked."** Owner follow-up: either set `flow=BUYBACK` on the staff create/quickBuy path (needs a UI/product decision on when a staff trade-in is a buyback vs an exchange) or accept the gap explicitly until then.
