...
| FIRST_STAFF_REPLY | ร้านตอบครั้งแรก (หลังทัก {x} นาที) | แชท/ติดต่อ | MIN(chat_messages.created_at) role=STAFF AND outbound_sent_at NOT NULL ข้ามข้อความทักทายตามกติกาเดียวกับ shouldSkipFirstOutboundClear (room-manager.service.ts) · แช่แข็งเป็น state.firstStaffReplyAt | — (จุดสัมผัส ไม่เลื่อนขั้น) | ร้าน | approximate |
...
- ผู้ซื้อที่ไม่ได้ผูกกับประวัติแชท (prod 0/2 คน สัญญา 8 ใบสร้างที่ร้าน) ทำให้การซื้อไม่ถูกนับเข้าช่องทางแชท — ทางแก้: คำใบ้ SamePerson แตะเดียวในหน้า POS/สร้างสัญญา (ใช้ absorb-into เดิม) + ไล่รวมมือหนึ่งรอบสำหรับผู้ซื้อหลักสิบคน
...
- รายการ one-off 'ผู้ซื้อที่ยังไม่ผูกแชท' ให้เจ้าของไล่กดรวม
