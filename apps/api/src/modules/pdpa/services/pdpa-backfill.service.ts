...
import { PdpaStatusService } from './pdpa-status.service';
import {
  errClass,
  idTail,
  plaintextWhere,
...
/**
 * Phase 3 SP4 — PDPA backfill orchestrator.
 *
 * Holds the advisory lock, the PdpaBackfillRun lifecycle, the encrypt/hash
 * field-pair plan + the single `$transaction`, the salvage loop, the W9
 * cursor-race retry, and Sentry capture. Reads PdpaStatusService for the
 * shared plaintext counts (cross-seam read, NOT inside the tx).
 *
 * Hard rule: **NEVER log decrypted PII** — only idTail / errClass / counts.
 */
@Injectable()
export class PdpaBackfillService {
  private readonly logger = new Logger(PdpaBackfillService.name);
  ...
  constructor(
    ...
    private readonly statusService: PdpaStatusService,
  ) {}
  ...
  async runBackfill(opts: RunBackfillOptions): Promise<BackfillResult> {
    ...
    const batchSize = opts.batchSize ?? PdpaBackfillService.DEFAULT_BATCH_SIZE;
    ...
    const lockResult = await this.prisma.$queryRaw<{ acquired: boolean }[]>`
      SELECT pg_try_advisory_lock(hashtext(${PdpaBackfillService.ADVISORY_LOCK_KEY})) AS acquired
    `;
    ...
    try {
      ...
      try {
        await this.prisma.$queryRaw`
          SELECT pg_advisory_unlock(hashtext(${PdpaBackfillService.ADVISORY_LOCK_KEY}))
        `;
      } catch (err) {
        this.logger.warn(`pdpa-backfill: advisory unlock failed: ${truncErr(err)}`);
      }
    }
  ...
  private async writeBackfillAuditLog(args: {
    ...
    try {
      ...
      this.logger.error(
        `pdpa-backfill: failed to write audit log: ${truncErr(err)}`,
      );
    }
  }
  ...
  private async runUnderLock(opts: {
    ...
    const totalRecords = await this.statusService.getAnyPlaintextCount();
    ...
    try {
      ...
      // window.
      const remaining = await this.statusService.getAnyPlaintextCount();
      if (remaining > 0) {
        this.logger.warn(
        ...
        const stillRemaining = await this.statusService.getAnyPlaintextCount();
        if (stillRemaining > 0) {
          racy = true;
          this.logger.warn(
  ...
  private async processBatch(
    ...
    for (const row of batch) {
      try {
        ...
        skipped++;
        skipReasons.push({ id: row['id'] as string, reason: errClass(err) });
      }
    }
    ...
    if (plans.length > 0) {
      try {
        ...
        this.logger.warn(
          `pdpa-backfill: batch tx failed (${errClass(err)}) — falling back to per-row writes`,
        );
        for (const p of plans) {
          try {
            ...
            skipped++;
            this.logger.warn(
              `pdpa-backfill: skipping customer (id-suffix=${idTail(p.id)}) — ${errClass(rowErr)}`,
            );
          }
        }
    ...
    for (const r of skipReasons) {
      this.logger.warn(
        `pdpa-backfill: skipping customer (id-suffix=${idTail(r.id)}) — ${r.reason}`,
      );
    }
