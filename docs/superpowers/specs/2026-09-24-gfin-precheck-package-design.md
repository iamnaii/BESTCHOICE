...
- วันที่: 2026-09-24
- สถานะ: **เจ้าของอนุมัติสเปก 2026-09-24** (mockup v6 + คำตอบ 5 ข้อ) · **PR 1 ทำแล้ว** บน `feat/gfin-precheck` (รวม fix wave จาก final review 2026-09-25 — ดูหัวข้อ "สถานะ" ท้ายไฟล์) · **PR 2 ทำแล้วบน `feat/gfin-precheck-pr2`**
...
2. LINE OA Manager ของ OA ไฟแนนซ์: เปิดอนุญาตเข้ากลุ่ม · ปิดทักทายในกลุ่ม · เชิญเข้ากลุ่ม GFIN : BESTCHOICE (ก่อนหรือหลัง PR 2 ขึ้นก็ได้ — ถ้าเชิญไว้ก่อน ระบบจะรู้จักกลุ่มเมื่อมีข้อความแรกในกลุ่มหลังอัปเดต)
...
- เบี่ยงเพิ่มเติมระหว่างดำเนินงาน PR 2: หน้าลิงก์สาธารณะ (footer) โชว์ชื่อกลุ่มไลน์จริงที่ผูกไว้ — แยก try/catch การค้นชื่อกลุ่มออกจากเส้นทางหลัก เพื่อไม่ให้ lookup ชื่อกลุ่มล้มแล้วพาให้ลิงก์ที่ยังใช้ได้กลายเป็นหน้า 410 ปลอม (fix round 1, T4) · `formatPrecheckTemplateErrors` อยู่ใน `packages/shared` จุดเดียว เพื่อให้ error 400 ของ API กับ toast ของหน้าตั้งค่าใช้ถ้อยคำเดียวกันเป๊ะ · ผจก.การเงิน (FINANCE_MANAGER) เข้าถึง ตั้งค่า › การเงิน › GFIN ได้แล้ว (ขยาย role ใน settings registry) แต่เห็นเฉพาะแท็บ "กลุ่มไลน์ & ข้อความ" — แท็บราคา/เรทยังเป็น OWNER เท่านั้น
...
- Task 7 (bump version · MCP grants · ตรวจทั้งชุด): web → `26.9.56` · รายละเอียดใน `.superpowers/sdd/2026-09-25-gfin-precheck-pr2/task-7-report.md`
