...
vi.mock('@/lib/api', () => ({ default: { get: vi.fn(), post: vi.fn() }, getErrorMessage: (error: Error) => error.message }));
...
const confirmed = close({ status: 'CONFIRMED', receivedAmount: 10510, destination: 'BANK_DEPOSIT', moneyState: 'REACHED',
  confirmedAt: '2026-09-21T13:52:00.000Z', confirmedBy: { id: 'u-owner', name: 'เจ้าของ' } });
...
const status = (over: Partial<CashCloseStatusResponse> = {}): CashCloseStatusResponse => ({
  date: '2026-09-21', asOf: '2026-09-21T13:00:00.000Z', branchId: 'br-1', branchName: 'ลพบุรี', round: emptyRound,
...
function renderHero(response: CashCloseStatusResponse) {
  ...
  render(
    <MemoryRouter><QueryClientProvider client={client}>
      <CashStatusHero status={response} date="2026-09-21" isToday onSend={vi.fn()} onConfirm={vi.fn()} onDeposit={vi.fn()} />
