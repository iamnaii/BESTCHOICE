...
export class PaymentReceiptOrchestrator {
  ...
  async recordPayment(
    ...
    const resolvedDepositAccountCode =
      depositAccountCode ?? (await resolveUserDefaultCashAccount(this.prisma, recordedById));
      ...
      async (tx) => {
        ...
        if (transactionRef) {
          ...
          const exactRefPattern = new RegExp(
            `ref:${transactionRef.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:\\s*\\||\\s*$)`,
          );
        ...
        if (!['ACTIVE', 'OVERDUE', 'DEFAULT'].includes(contract.status)) {
          throw new BadRequestException(
            'ไม่สามารถชำระเงินได้ สัญญาต้องอยู่ในสถานะ ACTIVE, OVERDUE หรือ DEFAULT',
          );
        }
        ...
        if (!payment.lateFeeWaived && payment.dueDate < effectivePaidDate) {
          const daysOverdue = Math.max(
            0,
            Math.floor(
              (effectivePaidDate.getTime() - payment.dueDate.getTime()) / (1000 * 60 * 60 * 24),
            ),
          );
        ...
        // rescheduleAdvanceBalance holds reschedule-fee (6a/6b) prepayments parked
        // for the contract's LAST installment ONLY — never FIFO'd into an earlier
        // installment the way the generic advance above is. isLastInstallment gates
        // every read/write of it below.
        const isLastInstallment = installmentNo === contract.totalMonths;
        let parkConsume = d(0);
        const beforePark = d(contract.rescheduleAdvanceBalance ?? 0);
        ...
        if (overage.gt(d('1.00'))) {
          ...
          const multCfg = await tx.systemConfig.findUnique({
            where: { key: 'overpay_advance_auto_max_multiplier' },
          });
          ...
          // Generic advance first, park bucket only for whatever gap remains —
          // and only on the contract's last installment.
          if (isLastInstallment && beforePark.gt(0)) {
            const gapAfterGeneric = gap.minus(advanceConsume);
            if (gapAfterGeneric.gt(0)) {
              parkConsume = Prisma.Decimal.min(beforePark, gapAfterGeneric);
            }
          }
        }
        ...
        const isPaidInFull =
          paymentCase === 'OVERPAY_ADVANCE' || advanceCredit.gt(0)
            ? true
            : dGte(recordedAmountPaid, amountDue);
        ...
        // Park-at-last-installment (owner directive 2026-08-16): consume Contract.
        // rescheduleAdvanceBalance when this payment (on the LAST installment)
        // covers a shortfall the generic advance above didn't fully cover. Separate
        // bucket, separate audit row — never touched on any other installment.
        if (parkConsume.gt(0)) {
          await tx.contract.update({
            where: { id: contractId },
          ...
          await tx.auditLog.create({
            data: {
              ...
              newValue: {
                paymentId: result.id,
                installmentNo,
                parkConsume: parkConsume.toString(),
                beforeParkBalance: beforePark.toString(),
                afterParkBalance: beforePark.minus(parkConsume).toString(),
                source: 'RECORD_PAYMENT_LAST_INSTALLMENT_PARK_CONSUME',
              },
            },
          });
        }
            ...
            const receiptPosted = await this.paymentReceiptTemplate.execute(
              {
                ...
                // Generic advance + park bucket both clear the SAME GL account
                // (21-1103) — the split between Contract.advanceBalance and
                // Contract.rescheduleAdvanceBalance is an application-level
                // bookkeeping distinction only, not a GL-level one.
                advanceConsume: advanceConsume.plus(parkConsume).gt(0)
                  ? advanceConsume.plus(parkConsume)
                  : undefined,
                ...
                autoApproveSystemRounding: dGte(
                  dAdd(d(amount), advanceConsume.plus(parkConsume)),
                  remaining,
                ),
              },
              tx,
  ...
  async autoAllocatePayment(
    ...
    const resolvedDepositAccountCode = await resolveUserDefaultCashAccount(
      this.prisma,
      recordedById,
    );
    ...
    const allocationResult = await this.prisma.$transaction(
      async (tx) => {
        ...
        const results: {
          updated: Awaited<ReturnType<typeof tx.payment.update>>;
        ...
        if (overpayment.gt(0)) {
          ...
          const referencePaymentId =
            results.length > 0 ? results[results.length - 1].updated.id : contractId;
          ...
          const depositCode =
            results.length > 0
              ? (results[results.length - 1].updated.depositAccountCode ??
        ...
        for (const payment of unpaid) {
          ...
          const updated = await tx.payment.update({
            ...
            data: {
              ...
              status: isPaidInFull ? 'PAID' : 'PARTIALLY_PAID',
              recordedById,
              notes: [payment.notes, `ใช้เครดิต ${payAmount.toNumber().toLocaleString()} บาท`]
                .filter(Boolean)
                .join(' | '),
            },
          });
