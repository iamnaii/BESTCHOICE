...
const selectTriggerVariants = cva(
  ...
  `,
  {
    variants: {
      size: {
        sm: 'h-7 px-2.5 text-xs leading-snug gap-1 rounded-md',
        md: 'h-[34px] px-3 text-[0.8125rem] leading-snug gap-1 rounded-md',
        lg: 'h-10 px-4 text-sm leading-snug gap-1.5 rounded-md',
      },
    },
    defaultVariants: {
      size: 'md',
  ...
  return (
    ...
    >
      ...
      <SelectPrimitive.Icon asChild>
        <ChevronDown className="h-4 w-4 opacity-60 -mr-0.5" />
...
function SelectLabel({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.Label>) {
  return (
    <SelectPrimitive.Label
      ...
      className={cn('py-1.5 pl-8 pr-2 text-xs text-muted-foreground font-medium', className)}
      {...props}
    />
  );
}
