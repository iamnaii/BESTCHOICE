#!/usr/bin/env bash
# ใช้คำตอบนโยบาย owner (2026-08-13) กับ 4 แถวสุดท้ายของคลังคำตอบ — รัน apply-kb-policy.sql
# รัน:  bash scripts/ops/apply-kb-policy.sh
set -u
cd "$(dirname "$0")/../.."
PORT=15432
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-kb-policy.sql || die "apply พัง"
ok "4 แถวนโยบายอัปเดต+เปิดใช้แล้ว — คลังคำตอบครบ 32/32"
