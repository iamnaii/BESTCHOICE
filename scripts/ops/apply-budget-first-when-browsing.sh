#!/usr/bin/env bash
# v2.17 ถามงบดาวน์ก่อนเมื่อลูกค้าเลือกไม่ถูก — รัน apply-budget-first-when-browsing.sql
# — รัน apply-budget-first-when-browsing.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-budget-first-when-browsing.sql || die "apply ล้มเหลว"
