...
try {
  ...
  const blobs=await page.evaluate(async ({long,company,date})=>{
   ...
   if(long) {
    for(const count of [8,12,16,20]) {
     result[`termination-boundary-${count}`]=await renderLetterPdf({letterType:'CONTRACT_TERMINATION_60D',letterNumber:`BOUNDARY-${count}`,letterDate:new Date(date),company:{...company,address:company.address.repeat(count)},customer:{name:'ลูกค้า ทดสอบขอบหน้า'},contract:{contractNumber:'CT-BOUNDARY',contractDate:new Date(date),outstanding:3150,daysOverdue:60},overdueDetail:{overdueMonths:Array.from({length:count},(_,i)=>`เดือนทดสอบลำดับ ${i+1}`),overdueInstallments:count,principalAmount:3000,lateFeeAmount:150}});
    }
   }
 ...
 await page.close();
 const fixtures=[
  ...['EXPENSE','PETTY_CASH_REIMBURSEMENT','PAYROLL'].map(type=>({name:`voucher-${type}`,url:'/expenses/specimen/voucher',api:'/expense-documents/specimen',data:{...base,documentType:type,...(type==='PETTY_CASH_REIMBURSEMENT'?{withholdingTax:'0',netPayment:'1070'}:{}),expenseDetail:{lines:rows.slice(0,2)},payroll:{payrollPeriod:'2026-09',lines:[{id:'employee',employeeName:'พนักงาน ตัวอย่าง',employeeTaxId:'0000000000000',baseSalary:'20000',ssoEmployee:'750',whtAmount:'200',netPaid:'19050'}]}}})),
  {name:'voucher-PAYROLL-custom',url:'/expenses/specimen/voucher',api:'/expense-documents/specimen',data:{...base,documentType:'PAYROLL',note:'หมายเหตุทดสอบสลิปเงินเดือน',payroll:{payrollPeriod:'2026-09',lines:[{id:'employee-1',employeeName:'พนักงาน รายได้หลายประเภท',employeeTaxId:'0000000000000',baseSalary:'20000',ssoEmployee:'750',whtAmount:'200',netPaid:'21550',customIncome:[{id:'ot',accountCode:'520102',name:'ค่าล่วงเวลา',amount:'1000',isTaxable:true},{id:'travel',accountCode:'520103',name:'ค่าเดินทางตามจริง',amount:'2000',isTaxable:false}],customDeduction:[{id:'loan',accountCode:'111200',name:'เงินยืมพนักงาน',amount:'500'}]},{id:'employee-2',employeeName:'พนักงาน ไม่มีรายการหัก',employeeTaxId:null,baseSalary:'15000',ssoEmployee:'0',whtAmount:'0',netPaid:'15000'}]}}},
 ...
 const daily=structuredClone(fixtures.find(f=>f.name==='expense-daily'));
 daily.name='expense-daily-many-buckets';
 daily.data.documents=Array.from({length:24},(_,i)=>({...base,id:String(i),number:`EXP-202609-${i}`,paymentMethod:'CASH'}));
 daily.data.grandTotal='25680';
 daily.data.byType={EXPENSE:{count:24,total:'25680'}};
 daily.data.byPaymentMethod={CASH:{count:24,total:'25680'}};
 daily.data.byCategory=Object.fromEntries(Array.from({length:24},(_,i)=>[`หมวดรายจ่าย ${i+1}`,{count:1,total:'1070'}]));
 daily.data.cashMovement=Object.fromEntries(Array.from({length:24},(_,i)=>[`บัญชีจ่าย ${i+1}`,{count:1,out:'1070'}]));
 fixtures.push(daily);
 for(const fixture of fixtures){
  ...
  await page.route('**/api/**',route=>{
   ...
   if(path==='/companies/public')return route.fulfill({json:{finance:company,shop:{...company,id:'preview-shop',nameTh:'บริษัท ร้านค้าทดสอบ จำกัด',companyCode:'SHOP',taxId:'1111111111111'}}});
  ...
  await page.evaluate('document.fonts.ready');
  const beforeStyles=await page.locator('.bc-document').evaluateAll(nodes=>nodes.map(n=>n.style.cssText));
  await page.pdf({path:`${out}/print-${fixture.name}.pdf`,format:'A4',preferCSSPageSize:true,printBackground:true});
  assert.deepEqual(await page.locator('.bc-document').evaluateAll(nodes=>nodes.map(n=>n.style.cssText)),beforeStyles,'Print must restore original inline styles');
  assert.equal(await page.locator('.bc-paper-relaxed').count(),0,'Print density class must be cleaned up');
  if(fixture.name==='voucher-EXPENSE') {
   await page.pdf({path:`${out}/print-voucher-repeat.pdf`,format:'A4',preferCSSPageSize:true,printBackground:true});
   const certificate=page.locator('.voucher-sheet').last();
   assert.match(await certificate.innerText(),new RegExp(company.nameTh));
   assert.ok(!(await certificate.innerText()).includes('บริษัท ร้านค้าทดสอบ จำกัด'));
  }
