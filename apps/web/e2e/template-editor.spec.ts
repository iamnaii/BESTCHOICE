...
test.describe('เทมเพลตสัญญา', () => {
  ...
  test('should display template list or empty state', async ({ page }) => {
    ...
    const hasTemplates = await page
      .locator('table tbody tr, .template-card, [data-testid="template-item"]')
      .first()
  ...
  test('should have create template button', async ({ page }) => {
    ...
    const createBtn = page
      .locator('button, a')
      .filter({ hasText: /สร้าง|เพิ่ม|Create|New/ })
      .first();
  ...
  test('should have PDF preview functionality', async ({ page }) => {
    ...
    const previewBtn = page
      .locator('button')
      .filter({ hasText: /ดูตัวอย่าง|Preview|PDF/ })
      .first();
...
test.describe('ราคาตั้งต้น', () => {
  ...
  test('should load pricing templates page', async ({ page }) => {
    if (await hasErrorBoundary(page)) return;
    await expect(page.getByText(/ราคาตั้งต้น|ราคามาตรฐาน|Pricing/).first()).toBeVisible({
      timeout: 15000,
    });
  });
  ...
  test('should have add pricing template button', async ({ page }) => {
    ...
    const addBtn = page
      .locator('button')
      .filter({ hasText: /เพิ่ม|สร้าง|Add|Create/ })
      .first();
  ...
  test('should filter by brand', async ({ page }) => {
    ...
    const brandFilter = page
      .getByPlaceholder(/ค้นหา|brand|ยี่ห้อ/i)
      .first()
  ...
  test('should have import/export functionality', async ({ page }) => {
    ...
    const importBtn = page
      .locator('button')
      .filter({ hasText: /Import|นำเข้า/ })
      .first();
    const exportBtn = page
      .locator('button')
      .filter({ hasText: /Export|ส่งออก/ })
      .first();
