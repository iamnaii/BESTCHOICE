...
# Repeated print must not accumulate spacing or move any text.
def geometry(name):
    with fitz.open(root / f'{name}.pdf') as document:
        return [[(s['text'], tuple(round(x, 2) for x in s['bbox']))
                 for b in page.get_text('dict')['blocks'] for line in b.get('lines', []) for s in line['spans']]
                for page in document]
assert geometry('print-voucher-EXPENSE') == geometry('print-voucher-repeat'), 'Repeated printing changed layout'
# Short forms use the lower part of A4 rather than clustering near the header.
for name in ['expense-normal','asset-normal','other-income-normal','receipt-PAYMENT-normal',
             'print-voucher-PETTY_CASH_REIMBURSEMENT','print-voucher-PAYROLL',
             'print-wht-annual-certificate','print-dividend-register-certificate']:
    with fitz.open(root / f'{name}.pdf') as document:
        bottom = max(s['bbox'][3] for b in document[0].get_text('dict')['blocks']
                     for line in b.get('lines', []) for s in line['spans'] if s['text'].strip())
        assert 700 < bottom < 795, f'{name}: unbalanced A4 bottom {bottom}'
print('PASS: A4 vertical occupancy and repeated-print geometry')

assert len(pages('report')) == 2, 'Overview and twenty follow-up rows must fit two pages'
