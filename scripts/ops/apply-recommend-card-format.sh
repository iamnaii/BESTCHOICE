#!/usr/bin/env bash
# v2.16 การ์ดสั้นรุ่นละแพ็ค + ลบ {customerName} — รัน apply-recommend-card-format.sql
# — รัน apply-recommend-card-format.sql (เขียนทับทั้งก้อนแบบ md5-guard)
...
psql "$PGURL" -v ON_ERROR_STOP=1 -f scripts/ops/apply-recommend-card-format.sql || die "apply ล้มเหลว"
